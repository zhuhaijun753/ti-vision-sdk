...
static void writeTrackInfoToCsvFile(strackInfo * trackInfo, uint16_t maxNumKeyPoints,uint32_t frameIdx)
{
  char trackInfoFile[200];
  TI_FILE * fp_track;
  int i,curTrackSize, curIndx, k, j;

  sprintf(trackInfoFile,"..\\testvecs\\output\\inTrackInfo_%d.csv",frameIdx);
...
int32_t sofProcess(IM_Fxns * handle, uint8_t * currImage, uint16_t * inKeyPoints, uint16_t * outKeyPoints, uint16_t * errEstPtr, uint16_t * trackInfo, uint16_t * numKeyPoints)
{
  int32_t status;
  ...
  for(frameIdx = params->startFrame; frameIdx < (params->numFrames + params->startFrame); frameIdx++)
  {
    ...
    */
    ...
    if((params->inPutType == 1) && (params->keyPointDetectMethod == SOF_EXTERNAL_KEY_POINTS) && (frameIdx == 1))
    {
      ...
      profiler_start();
      status = sofProcess(sofAlgHandle,imagePtr[0], inKeyPoints, outKeyPoints, errEstPtr, (uint16_t *)trackInfo,&numKeyPoints);
...
#if ENABLE_TRACK_DUMP
    writeTrackInfoToCsvFile(trackInfo, params->maxNumKeyPoints, frameIdx);
#endif

#if ENABLE_FLOW_VISUVALIZATION
    if(strcmp(params->outVideoSeqFile,"NA"))
    {
      APP_drawFlow(imagePtr[0],params->outVideoSeqFile,trackInfo, params->maxNumKeyPoints, params->imWidth, params->imHeight,params->startX,params->startY, firstFrame);
    ...
    */
    if(params->inPutType == 1)
    {
      ...
      */
      if(frameIdx == 1)
      {
        writeOutputToTxtFile(trackInfo,params->maxNumKeyPoints,params->startX,params->startY, params->outFileName);
