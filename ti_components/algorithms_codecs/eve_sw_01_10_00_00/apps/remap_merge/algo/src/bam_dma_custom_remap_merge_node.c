...
#define ALIGN_SHORT(a)  (((a) + 2U  - 1U) & ~(2U - 1U))
                       ...
                       void * edmaContext)
{
    ...
    if(context->enableTileApproach)
    {
        ...
        if (srcFormat == YUV_420SP) {
            ...
            scatterGatherParams->dstPtr[i]       = *(context->intImageBlkPtr) + ALIGN_TLU(inputTileSize);
        ...
        scatterGatherParams->blkHeight[i]    = 1;
        i++;
        ...
        scatterGatherParams->dstPtr[i]       = *(uint8_t (*)[])(void *)scatterGatherParams->dstPtr[i - 1U] +
                                               sizeof(sTileLutHeader) + (ALIGN_TLU(maxNumPixelsinTile)*sizeof(uint16_t));
        ...
        i++;
        context->extLUTRunningPtr = (uint8_t (*)[])(*(context->extLUTRunningPtr) +
                                    (ALIGN_SHORT(tileInfo->numPixels)*sizeof(uint8_t)));

        /* EDMA configuration for output scatter offset indexes. */
        scatterGatherParams->srcPtr[i]       = *(context->extLUTRunningPtr);
        scatterGatherParams->dstPtr[i]       = *(uint8_t (*)[])(void *)scatterGatherParams->dstPtr[i - 1U] +
                                               (ALIGN_TLU(maxNumPixelsinTile)*sizeof(uint8_t));
    ...
    } else
    {
        ...
        if (srcFormat == YUV_420SP) {
            ...
            scatterGatherParams->dstPtr[1]       = *(context->intImageBlkPtr) + ALIGN_TLU(params->maps.maxInputBlockSize);
            ...
            scatterGatherParams->blkHeight[1]    = blockInfo->inputBlockHeight >> 1;
        }
                      ...
                      void * edmaContext)
{
    ...
    (*scatterGatherWidthList)[i] = (3 + tileInfo->numPixels)*sizeof(uint16_t);
    i++;
    context->extLUTRunningPtr    = (uint8_t (*)[])(*(context->extLUTRunningPtr) +
                                   ((3 + tileInfo->numPixels)*sizeof(uint16_t)));
                      ...
                      void * edmaContext)
{
    ...
    if (context->dmaArgs.remapParams.maps.srcFormat == YUV_420SP) {
        ...
        (*(uint16_t (*)[])(void *)inputImageTransferProp->blkHeight)[1]    = blockInfo->inputBlockHeight >> 1;
    }
