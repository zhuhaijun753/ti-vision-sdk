        ...
        const IVISION_InBufs *inBuf, const IVISION_OutBufs *outBuf , uint8_t sinkNodeId)
{
    ...
    status = BAM_controlNode(graphHandle, 0, (void *)inBuf);
    ...
    status= BAM_controlNode(graphHandle, sinkNodeId, (void *)outBuf);
...
/*RESET_MISRA("16.7")  -> Reset rule 16.7  */
{
    ...
    *remapKernelArgs         = *remapParams;
    ...
    if(srcFormat == YUV_420SP) {
        ...
        formatConvertKernelArgs->out_stride  = blockWidth << 1U;
    }
    else if(srcFormat == YUV_422ILE) {
        formatConvertKernelArgs->in_stride   = blockWidth << 1U;
    ...
    if (dstFormat == YUV_420SP) {
        ...
        alphaBlendKernelArgs->in_img1_stride = blockWidth << 1U;
        alphaBlendKernelArgs->in_img2_stride = blockWidth << 1U;
        alphaBlendKernelArgs->out_stride     = blockWidth << 1U;
    }
    else
    {
