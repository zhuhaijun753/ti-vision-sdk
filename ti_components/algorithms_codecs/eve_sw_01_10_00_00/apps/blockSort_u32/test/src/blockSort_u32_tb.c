   ...
   -----------------------------------------------------------------*/
    ...
    for (t=0;t<blockSort_u32_numTestCases;t++) {
        ...
        TEST_PRINTF("%2d) Block Sort %5d points    %10s %10d %10d %15.2f %20.2f %10.2f %20.2f %16.2f       Create graph is an additional %2.2f Mcycles per frame but it is a one-time startup call\n", testnum++, createParams.blockWidth*createParams.blockHeight,
                ...
                (2*profile_tsc/1000000.0), 30*(2*profile_tsc/1000000.0), (profile_tsc*2.0)/(gConfig_blockSort_u32[t].srcImageWidth * gConfig_blockSort_u32[t].srcImageHeight),
                (profile_sctm*2.0)/(gConfig_blockSort_u32[t].srcImageWidth * gConfig_blockSort_u32[t].srcImageHeight),(2*profile_tsc/500000.0), (profile_create_tsc*2.0)/1000000.0);
