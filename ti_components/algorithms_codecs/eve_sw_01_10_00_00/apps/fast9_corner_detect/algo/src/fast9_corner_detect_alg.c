...
{
  {
    (void *)&FAST9_CORNER_DETECT_TI_VISION_FXNS, /* module ID */
...
const IRES_Fxns FAST9_CORNER_DETECT_TI_IRES = {
  (void *)&FAST9_CORNER_DETECT_TI_IRES,
                                                  ...
                                                  IALG_MemRec memRec[])
{
  ...
  FAST9_CORNER_DETECT_TI_CreateParams * createParams =
    (FAST9_CORNER_DETECT_TI_CreateParams*)(void *)params;
  ...
  */
  ...
  memRec[BIN_IMG_SCRATCH_MEMREC].size       = ((createParams->imgFrameWidth[0] +7)/8)*
                                                   ...
                                                   const IALG_Params * params)
{
  ...
  for (levelCnt = 0 ; levelCnt < createParams->numLevels; levelCnt++)
  {
    ...
    algHandle->dx[levelCnt] = (createParams->imgFrameWidth[levelCnt] -
                                         algHandle->activeImageWidth[levelCnt])/2 ;
    algHandle->dy[levelCnt] = (createParams->imgFrameHeight[levelCnt] -
                                          algHandle->activeImageHeight[levelCnt])/2 ;
    ...
    if(extra_pixels_in_border > 0) {
        algHandle->dy_binImg[levelCnt] = extra_pixels_in_border;
  ...
  algHandle->ivision = (IVISION_Fxns *)&FAST9_CORNER_DETECT_TI_VISION_FXNS;
  ...
  for ( levelCnt = 0 ; levelCnt < intAlgHandle->numLevels; levelCnt++)
  {
    if ( fast9InArgs->levelMask & (uint8_t)(1U << levelCnt ))
    {
      ...
      if ( bamStatus == BAM_S_SUCCESS )
      {
        ...
        binImg2ListCtrlArgs.blkStride       = 8*((intAlgHandle->blockWidthBinImgToList[levelCnt]+7)/8);
      ...
      if (outArgs->size == sizeof (FAST9_CORNER_DETECT_TI_OutArgs))
      {
        ...
        fast9OutArgs->numCorners[levelCnt]        = numCorners;
      }
    }
  }
