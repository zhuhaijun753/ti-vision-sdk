...
{
        {
                (void *)&FILTER_2D_TI_VISION_FXNS, /* module ID */
        ...
        const IALG_Params * params)
{
    ...
    algHandle->createParams = *((FILTER_2D_CreateParams*)(void *)params);
    ...
    algHandle->ivision = (IVISION_Fxns *)&FILTER_2D_TI_VISION_FXNS;
...
{
  int32_t status = IALG_EOK;
  IVISION_InBufs *inBufs = (IVISION_InBufs*)(void *)inParams;
...
/* RESET_MISRA("16.7")  -> Reset rule 16.7     */
    ...
    if ( intAlgHandle->createParams.enableContrastStretching)
    {
      ...
      if (inArgs->size == sizeof (FILTER_2D_InArgs))
      {
        ...
        if ( (maxVal - minVal) != 0 )
        {
          scaleFactorQ16 = (uint16_t)(( (uint32_t)1U << 16U) / (( (uint32_t)maxVal - (uint32_t)minVal)));
        }
        else
        {
          scaleFactorQ16 = (uint16_t)(1U << 16U);
        }
