...
/*     and is used in this file, by vcop_BayerCFA_HorzUpsample_general_cn() */
...
#include "../inc/vcop_BayerCFA_HorzUpsample_c.h"
...
)                                                                                        
{
    ...
    for ( i = 0; i < (int) h_compute; i++) // lpend1
    {
        ...
        for (j = 0; j < (int) w_compute; j+=2) //lpend2
        {

            for ( k = 0; k < 2; k++) // for Bayer CFA skip by 2
            {
            ...
            result = 0;
                ...
                out[(i * w_out) + (j * 1 * U) + (m * 2) + k] = (unsigned short) result;
            }
