...
)
{
   ...
   int prev_min, prev_max, median, prev_mid;
   int prev_minus0, prev_minus1, prev_minus2, prev_minus3, prev_minus4;
   int tmp;
   int t_minus0, t_minus1, t_minus2, t_minus3, t_minus4, t2;
   int in;

   /*--------------------------------------------------------------------*/
   /* This is the code for computing row-wise median taking three pixels */
   /* at a time.                                                         */
   /*--------------------------------------------------------------------*/

   if (median_size == 3)
   {
       for ( i = 0; i < compute_height; i++)
       {
           ...
           for ( j = 0; j < compute_width; j++)
           {
               for ( k = 0; k < median_size; k++)
               {
                   prev_max = input_ptr[i * input_width + j + k];
               ...
               output_ptr[i * output_width + j] = t_mid;
           }
       }
   }

   /*--------------------------------------------------------------------*/
   /* This is the code for computing row-wise median taking five pixels  */
   /* at a time.                                                         */
   /*--------------------------------------------------------------------*/
       ...
       for ( i = 0; i < compute_height; i++)
       {
           ...
           for ( j = 0; j < compute_width; j++)
           {
               for ( k = 0; k < median_size; k++)
               {
                   ...
                   prev_minus0 = input_ptr[i * input_width + j + k];
                   
#ifdef __PRINT__

                   printf("IN: %d, %d, %d, %d, %d\n", prev_minus4, prev_minus3, prev_minus2, prev_minus1, prev_minus0);
#endif
...
#ifdef __PRINT__
               printf("%d, %d, %d, %d, %d\n", t_minus4, t_minus3, t_minus2, t_minus1, t_minus0);
               printf("%d\n\n", t_minus2);
#endif
               }
               
               output_ptr[i * output_width + j] = t_minus2;
           }
       }
   }
