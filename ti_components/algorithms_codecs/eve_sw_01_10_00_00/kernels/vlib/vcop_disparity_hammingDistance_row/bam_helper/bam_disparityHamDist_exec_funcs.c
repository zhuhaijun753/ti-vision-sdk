...
*/
    ...
    status= vcop_disparity_hammingDistance32_nth_row_firstCall_getPtr(
            ...
            rightPrevRowOffset,
            rightLastRowOffset,
            (uint8_t)(sizeof(leftPrevRowOffset)/2),
            context->kernelArgs.direction
    );
    ...
    /* Used to calculate address */
    for(i=0;i<5U;i++) {
        context->nth_row_firstCall_leftPrevRowPtr[0][i]= (uint32_t*)(void*)context->pNthRowFirstCallParams[0] + (leftPrevRowOffset[i]/2);
        context->nth_row_firstCall_leftLastRowPtr[0][i]= (uint32_t*)(void*)context->pNthRowFirstCallParams[0] + (leftLastRowOffset[i]/2);
        context->nth_row_firstCall_rightPrevRowPtr[0][i]= (uint32_t*)(void*)context->pNthRowFirstCallParams[0] + (rightPrevRowOffset[i]/2);
        context->nth_row_firstCall_rightLastRowPtr[0][i]= (uint32_t*)(void*)context->pNthRowFirstCallParams[0] + (rightLastRowOffset[i]/2);
    }
     ...
     */
    for(i=0;i<4U;i++) {
        context->nth_row_leftPrevRowPtr[0][i]= *(uint32_t(*)[])(void*)context->pNthRowParams[0] + (leftPrevRowOffset[i]/2);
        context->nth_row_leftLastRowPtr[0][i]= *(uint32_t(*)[])(void*)context->pNthRowParams[0] + (leftLastRowOffset[i]/2);
        context->nth_row_rightPrevRowPtr[0][i]= *(uint32_t(*)[])(void*)context->pNthRowParams[0] + (rightPrevRowOffset[i]/2);
        context->nth_row_rightLastRowPtr[0][i]= *(uint32_t(*)[])(void*)context->pNthRowParams[0] + (rightLastRowOffset[i]/2);
    }
