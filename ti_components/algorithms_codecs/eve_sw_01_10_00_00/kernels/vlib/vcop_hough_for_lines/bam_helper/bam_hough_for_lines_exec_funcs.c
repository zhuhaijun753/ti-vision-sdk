...
static BAM_Status Bam_HoughForLines_initFrame(void *kernelContext)
{
  ...
  interimTransposeStride = ((( (ALIGN_2SIMD(context->kernelArgs.rhoMaxLength) *
  ...
  tempData = cosLutQ8[context->currThetaIdx] * context->normalizationFactorQ8;
  (*pCosSinMulNorm)[0] = (int16_t) (tempData / 2) ;

  tempData = sinLutQ8[context->currThetaIdx] * context->normalizationFactorQ8;
