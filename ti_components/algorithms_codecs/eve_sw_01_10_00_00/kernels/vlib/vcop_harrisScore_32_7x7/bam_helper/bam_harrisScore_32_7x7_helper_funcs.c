...
#endif
    internalBlock[XX_IDX].size = (uint32_t)sizeof(uint32_t)*(ALIGN_SIMD(args->width)*(args->height+1u));
    internalBlock[XX_IDX].attrs.memAttrs = BAM_MEMATTRS_SCRATCH;

    internalBlock[YY_IDX].size = (uint32_t)sizeof(uint32_t)*(ALIGN_SIMD(args->width)*(args->height+1u));
    internalBlock[YY_IDX].attrs.memAttrs = BAM_MEMATTRS_SCRATCH;

    internalBlock[XY_IDX].size = (uint32_t)sizeof(uint32_t)*(ALIGN_SIMD(args->width)*(args->height+1u));
