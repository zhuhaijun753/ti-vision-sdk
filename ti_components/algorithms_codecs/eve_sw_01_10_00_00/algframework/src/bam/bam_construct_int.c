...
BAM_Status BAM_getGraphMemSize(const BAM_NodeParamsInt nodeList[], const BAM_EdgeParams edgeList[], uint32_t *graphMemSize, uint32_t *numMemtabs)
{
    ...
    *numMemtabs = 0;
    ...
    while(nodeList[i].nodeIndex!= BAM_END_NODE_MARKER)
    {
        *graphMemSize += BAM_ALIGN_WORD(nodeList[i].kernelInfo->kernelContextSize);
         ...
         */
        if ((nodeList[i].kernelInfo->coreType== BAM_EVE_ARP32) && (nodeList[i].kernelInfo->nodeType== BAM_NODE_COMPUTE)) {
            *graphMemSize += BAM_ALIGN_WORD(nodeList[i].kernelInfo->kernelContextSize);
        }
    ...
    *graphMemSize += (sizeof(BAM_GraphDef) + (sizeof(BAM_NodeDef) * totalNodes) + (sizeof(BAM_DataBlockDef) * numDataBlocks));
...
BAM_Status BAM_updateNodes(BAM_GraphHandle graphHandle, BAM_CreateGraphParams *createParams, BOOL callSetMemRec){
    ...
    if (callSetMemRec== (BOOL)true) {
        status = BAM_setMemRec(graphHandle, graphDef->memRec, createParams->nodeList);
        if (status != BAM_S_SUCCESS) {
            goto Exit;
        }
    }
...
static BAM_Status BAM_autoAllocMemRec(BAM_GraphHandle graphHandle, BAM_CreateGraphParams *createParams, BAM_MemRec memTab[])
{
    ...
    vcop_minit();
#endif
    bam_minit(packetPool, graphDef->numMemTabs + BAM_NUM_GUARD_PACKETS);
    ...
    for (kernelIndex = 0; kernelIndex < graphDef->totalNodes; kernelIndex++) {
        ...
        for (j=0; j < numRecInternal; j++) {
            ...
            if ((memTab[curMemTab].space== BAM_MEMSPACE_WBUF) && ((memTab[curMemTab].attrs.memAttrs== BAM_MEMATTRS_PERSIST) || (memTab[curMemTab].attrs.memAttrs== BAM_MEMATTRS_CONST))) {
                ...
                status= BAM_getMemPacket(&packetPtr, (VCOP_MEMHEAP)memTab[curMemTab].space, 32, size);
        ...
        for (j=0; j < (numRecInternal + numRecOutputDataBlock); j++) {
            ...
            if (!((memTab[curMemTab].space== BAM_MEMSPACE_WBUF) && ((memTab[curMemTab].attrs.memAttrs== BAM_MEMATTRS_PERSIST) || (memTab[curMemTab].attrs.memAttrs== BAM_MEMATTRS_CONST)))) {
                ...
                if(space < IBAMALG_MEMHEAP_DMEM) {
                    ...
                    status= BAM_getMemPacket(&packetPtr, (VCOP_MEMHEAP)space, 32, size);
                    ...
                    if (status == BAM_E_BLOCK_DIM_TOO_BIG) {
                        ...
                        if (space== BAM_MEMSPACE_WBUF) {
                            useWMEM= (BOOL)false; /* For now on, we avoid using WMEM since allocation failed one time, likely due to big constant tables in WMEM */
...
#ifdef DEBUG
                    PRINTF("[BAM node #%d] BAM_autoAllocMemRec() Invalid memory record !!!\n", nodeIndex);
#endif
                    goto ExitLoop;
                }
...
static BAM_Status BAM_newNode(BAM_GraphHandle graphHandle, const BAM_KernelInfo *kernelInfo, uint8_t *nodeId)
{
     ...
     * BAM_finaliseGraph() will be in charge to create both versions of each pointer.
     */
    if (((*nodeArr)[nodeIndex].kernelInfo.coreType== BAM_EVE_ARP32) && ((*nodeArr)[nodeIndex].kernelInfo.nodeType== BAM_NODE_COMPUTE)) {
        graphDef->curFreeKernelContextOffset += BAM_ALIGN_WORD((*nodeArr)[nodeIndex].kernelInfo.kernelContextSize);
    }
        ...
        uint8_t nodeId, uint8_t inputOutputBlockIndex)
{
    ...
    if (nodeId != BAM_NULL_NODE) { /* if nodeId is BAM_NULL_NODE, skip the section of code below */
        ...
        if ( (BAM_NODE_SOURCE != nodeType) && (BAM_NODE_SINK != nodeType) && (BAM_NODE_COMPUTE != nodeType))
        {
            /* Not possible */
            status = BAM_E_INVALID_NODE_TYPE;
            goto Exit;
...
static BAM_Status BAM_getMemRec(BAM_GraphHandle graphHandle, const BAM_CreateGraphParams *createParams, BAM_MemRec memTab[])
{
    ...
    for (kernelIndex = 0; kernelIndex < graphDef->totalNodes; kernelIndex++)
    {
        ...
        for (memRecIndex = 0; memRecIndex < numRecInternal; memRecIndex++) {
            memRecInternal[memRecIndex].alignment= 32;  /* To match the alignment of vcop_malloc() */
        ...
        for (memRecIndex = 0; memRecIndex < numRecOutputDataBlock; memRecIndex++) {
            memRecOutputDataBlock[memRecIndex].alignment= 32; /* To match the alignment of vcop_malloc() */
...
/* algInit */
static BAM_Status BAM_setMemRec(BAM_GraphHandle graphHandle, const BAM_MemRec memTab[], const BAM_NodeParams nodeList[])
{
    ...
    status = BAM_finaliseGraph(graphDef, nodeList);
...
static BAM_Status BAM_finaliseGraph(const BAM_GraphDef *graphDef, const BAM_NodeParams nodeList[])
{
    BAM_Status status = BAM_S_SUCCESS;
    ...
    for (nodeIndex = 0; nodeIndex < graphDef->totalNodes; nodeIndex++)
    {
        ...
        sizePrivContext= ((*node)[nodeIndex]).kernelInfo.kernelContextSize - ((uint32_t)&((*pPrivContext)[0]) - (uint32_t)&((*context)[0]));
        ...
        if ((*node)[nodeIndex].nodeType == BAM_NODE_COMPUTE) {
            ...
            if ((*node)[nodeIndex].kernelInfo.coreType== BAM_EVE_ARP32) {
                ...
                alignedSize= BAM_ALIGN_WORD((*node)[nodeIndex].kernelInfo.kernelContextSize)/4U;
                ...
                pContextArgs_b= (uint8_t (*)[])(void*)&((*pContextArgs)[(alignedSize*4U)]);
                pPrivContext_b= (uint8_t (*)[])(void*)&((*pPrivContext)[(alignedSize*4U)]);
                ...
                for (dataBlkIndex = 0; dataBlkIndex < (*node)[nodeIndex].kernelInfo.numRecInternal; dataBlkIndex++) {
                    ...
                    if (memRecInternalDataBlock[dataBlkIndex]->space < BAM_MEMSPACE_WBUF) {
                        ...
                        (*pInternalBlockArray_b)[dataBlkIndex]= (void*)&((*pImBuf)[BAM_IMGBUF_GAP]);
                ...
                for (dataBlkIndex = 0; dataBlkIndex < (*node)[nodeIndex].kernelInfo.numInputDataBlocks; dataBlkIndex++) {
                    ...
                    if (memRecInputDataBlock[dataBlkIndex]->space < BAM_MEMSPACE_WBUF) {
                        ...
                        (*pInBlockArray_b)[dataBlkIndex]= (void*)&((*pImBuf)[BAM_IMGBUF_GAP]);
                    }/* if the pointer is not imbuf, still need to fill pInternalBlockArray_b, otherwise it would contain garbage */
                    else {
                        (*pInBlockArray_b)[dataBlkIndex]= (*pInBlockArray)[dataBlkIndex];
                    }
                ...
                for (dataBlkIndex = 0; dataBlkIndex < (*node)[nodeIndex].kernelInfo.numOutputDataBlocks; dataBlkIndex++) {
                    ...
                    if (memRecOutputDataBlock[dataBlkIndex]->space < BAM_MEMSPACE_WBUF) {
                        ...
                        (*pOutBlockArray_b)[dataBlkIndex]= (void*)&((*pImBuf)[BAM_IMGBUF_GAP]);
                    }/* if the pointer is not imbuf, still need to fill pInternalBlockArray_b, otherwise it would contain garbage */
                    else {
                        (*pOutBlockArray_b)[dataBlkIndex]= (*pOutBlockArray)[dataBlkIndex];
                    }
        ...
        if ((*kernelHostDbArray)[kernelId].kernelHelperFunc->setKernelMemRec != NULL) {
            ...
            status= (*kernelHostDbArray)[kernelId].kernelHelperFunc->setKernelMemRec(memRecInternalDataBlock, memRecInputDataBlock,
                    memRecOutputDataBlock, (*node)[nodeIndex].kernelInfo.numRecInternal, (*node)[nodeIndex].kernelInfo.numInputDataBlocks,
...
BAM_Status BAM_optimizeGraph(BAM_GraphHandle graphHandle, BAM_CreateGraphParams *createGraphParams) {
    ...
    BOOL bestBlockWidthFound= false;
    BOOL bestBlockHeightFound= false;
     ...
     * If we don't do that and enter the loop while (bestBlockWidthFound== (BOOL)false), then in the first call to createGraphParams->initKernelsArgsFunc()
    ...
    if (blockDimParams->blockWidthStep== 0) {
        bestBlockWidthFound= (BOOL)true;
    }

    /* CHECK_MISRA("-14.6")  -> Disable rule 14.6 */
    /* Iteration statement has more than one break or goto for loop termination */
    /* Deviation is allowed because all the GOTOs point to a single exit point */
    while (bestBlockWidthFound== (BOOL)false)  {
         ...
         * which is usually the processing ROI's width. This is because currently the EDMA can only fetch a number of blocks that is exact divisor
         * of the ROI's width or height.
        ...
        if ((status== BAM_E_BLOCK_DIM_TOO_BIG) || (blockDimParams->blockWidth > blockDimParams->blockWidthDivisorOf)) {
            bestBlockWidthFound= (BOOL)true;
    ...
    if (blockDimParams->blockHeightStep== 0) {
        bestBlockHeightFound= (BOOL)true;
    }

    while (bestBlockHeightFound== (BOOL)false)  {
        ...
        if ((status== BAM_E_BLOCK_DIM_TOO_BIG) || (blockDimParams->blockHeight > blockDimParams->blockHeightDivisorOf)) {
            bestBlockHeightFound= (BOOL)true;
    ...
    status= BAM_updateNodes(graphHandle, createGraphParams, true);
    if (status != BAM_S_SUCCESS) {
        goto Exit;
    }
