...
static uint8_t ibuflamem[IMEML_SIZE];
static uint8_t ibufhamem[IMEMH_SIZE];
static uint8_t ibuflbmem[IMEML_SIZE];
static uint8_t ibufhbmem[IMEMH_SIZE];
static uint8_t wbufmem[WMEM_SIZE];
...
int32_t bam_minit(BAM_Packet *packet_pool, int32_t num_packets)
{
    ...
    /* CHECK_MISRA("-8.1")  -> Disable rule 8.1 */
    /* Function has not prototype */
    /* The macro OVERHEAD uses the macro offsetof which in turn uses __intaddr__ */
    /* Misra complains about __intaddr__ by saying it doesn't have any prototype, however deviation is allowed because the function  __intaddr__ is an ARP32 compiler defined function.*/
    /* CHECK_MISRA("-20.1 -20.5 -20.6 -20.7 -20.8")  -> Disable rules*/
    /* Unsafe macro usage */
    /* The macro OVERHEAD makes use of offsetof, which calls __intaddr__. Per the previous rule, it is safe to call this macro so a deviation is allowed. */
    bam_overhead= OVERHEAD;
    /* RESET_MISRA("8.1")  -> Reset rule 8.1*/
    /* RESET_MISRA(20.1 20.5 20.6 20.7 20.8")  -> Reset rule -20.1 -20.5 -20.6 -20.7 -20.8*/
    ...
    for (heap = VCOP_IBUFLA; heap <= VCOP_WMEM; heap++)
    {
        ...
        temp2= - ((int32_t)temp - bam_overhead);
        size= (memsz_t)temp2;
...
BAM_Packet *bam_getMemPacket(VCOP_MEMHEAP heap, int32_t alignment, size_t size)
{
     ...
     *-----------------------------------------------------------------------*/
    allocsize += (alignment + sizeof(void*));
...
/* Need to update MISRA-C policy document: Deviation allowed since compiler preserved the signed bit when right shifting signed integer */
    allocsize= (((allocsize + MINSIZE_MINUS_1)>>MINSIZE_SHIFT)<<MINSIZE_SHIFT) - bam_overhead;
/* RESET_MISRA("12.7")  -> Reset rule 12.7*/
     ...
     *-----------------------------------------------------------------------*/
    packet = bam_mem_alloc(&vcop_mem[heap].freeblk, (size_t)allocsize, heap);
    ...
    if ( (vcop_view == VCOP_MEMALIASED) && (vcop_mem[heap].aliasidx != VCOP_NONE))
    {
        ...
        BAM_Packet *apacket = bam_mem_alloc(&vcop_mem[aheap].freeblk, allocsize, aheap);
    ...
    index= bam_overhead + (int32_t)sizeof(void*);
    addr= &((*uaptr)[index]);
    ...
    addr_int= ((int32_t)addr + alignment_1) & (~alignment_1);
    /* RESET_MISRA("12.7")  -> Reset rule 12.7*/
...
BAM_Packet *bam_mem_alloc(BAM_Packet **free_mem, size_t allocsize, int32_t heap)
{
    ...
    if ((int32_t)(-current->packet_size) > (allocsize + bam_overhead + MINSIZE))
    {
        /*-------------------------------------------------------------------*/
        ...
        next->packet_size=current->packet_size+allocsize+bam_overhead;/*NEG==FREE*/
        ptr= (uint8_t (*)[])(void*)current->uaptr;
        next->uaptr= (void*)&((*ptr)[allocsize + bam_overhead]);
...
void bam_mem_free(BAM_Packet **free_mem, BAM_Packet *packet)
{
    ...
    /* address.								     */
    /*-----------------------------------------------------------------------*/
    while ((next->next_free != LIMIT) && ((uint8_t*)next->uaptr < (*sysblock)))
    {
        prev = next;
        next = next->next_free;
    ...
    /*-----------------------------------------------------------------------*/
    if (&((*sysblock)[packet->packet_size + bam_overhead]) == (uint8_t *)next->uaptr)
    {
        packet->next_free = next->next_free;
        packet->packet_size=  (packet->packet_size - next->packet_size) + bam_overhead;/* POS==USED */
    ...
    if (prev)               /* ARE WE THE NEW HEAD OF THE LIST */
    {
        ...
        if ( &((*uaptr)[- prev->packet_size + bam_overhead]) == (*sysblock))
        {
            prev->next_free = packet->next_free;
            prev->packet_size = (prev->packet_size -packet->packet_size) - bam_overhead;/*NEG==FREE*/
