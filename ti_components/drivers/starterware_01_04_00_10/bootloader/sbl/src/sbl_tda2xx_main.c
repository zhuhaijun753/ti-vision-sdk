 ...
 *   Component:     SBL
 *
 *   Filename:      sbl_tda2xx_main.c
 *
 *   Description:   This file contain main function, call the Soc Init
 ...
 * =============================================================================
 */
uint32_t entryPoint_MPU_CPU0  = 0, entryPoint_MPU_CPU1 = 0;
uint32_t entryPoint_IPU1_CPU0 = 0, entryPoint_IPU1_CPU1 = 0;
uint32_t entryPoint_IPU2_CPU0 = 0, entryPoint_IPU2_CPU1 = 0;
uint32_t entryPoint_DSP1      = 0, entryPoint_DSP2 = 0;
uint32_t entryPoint_EVE1      = 0, entryPoint_EVE2 = 0, entryPoint_EVE3 = 0,
         entryPoint_EVE4      = 0;
...
/**
 * \brief   main function is SBL second level entry point. It sequences the Soc
 *           Init & Slave core boot-up. Get called from Entry function
 *
 *
 * \param   None.
 *
...
int main(void)
{
    ...
    /* AVS-0 and IO delay Recalibration has already been done in NOR bootmode */
#if (defined (QSPI) || defined (MMCSD))
    ...
    MPU_CPU1_BringUp(entryPoint_MPU_CPU1, sblBuildMode);
    SBLIPUBringUp((uint32_t) 0, entryPoint_IPU1_CPU0, entryPoint_IPU1_CPU1,
                  sblBuildMode);
    SBLIPUBringUp((uint32_t) 1, entryPoint_IPU2_CPU0, entryPoint_IPU2_CPU1,
                  sblBuildMode);

    DSP1_BringUp(entryPoint_DSP1, sblBuildMode);
    DSP2_BringUp(entryPoint_DSP2, sblBuildMode);

    EVE1_BringUp(entryPoint_EVE1, sblBuildMode);
    EVE2_BringUp(entryPoint_EVE2, sblBuildMode);
    EVE3_BringUp(entryPoint_EVE3, sblBuildMode);
    EVE4_BringUp(entryPoint_EVE4, sblBuildMode);

#ifdef ZEBU
    IVAHD_BringUp(0, sblBuildMode);
#endif
