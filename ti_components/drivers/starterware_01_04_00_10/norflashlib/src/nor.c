...
NOR_InfoHandle NOR_open(uint32_t baseCSAddr, uint8_t busWidth)
{
    ...
    else
    {
        ...
        if (LOCAL_flashQueryCFI(hNorInfo) != SUCCESS)
        {
                   ...
                   volatile uint32_t size)
{
    ...
    while (addr < range)
    {
        if (NOR_getBlockInfo(hNorInfo, addr, &blockSize, &blockAddr) != SUCCESS)
        {
        ...
        else
        {
            ...
            if ((*Flash_Erase)(hNorInfo, blockAddr) != SUCCESS)
            {
