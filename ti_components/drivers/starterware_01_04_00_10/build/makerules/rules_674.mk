...
CFLAGS_INTERNAL = -c -qq -pdsw225 --endian=$(ENDIAN) -mv6740 --abi=$(CSWITCH_FORMAT) -eo.$(OBJEXT) -ea.$(ASMEXT) -mi10 -mo -pdr -pden -pds=238 -pds=880 -pds1110 --program_level_compile -g
...
ifndef MODULE_NAME
  ...
  PKG_LIST_C67X_LOCAL = $(foreach COMP,$(COMP_LIST_$(CORE)),$($(COMP)_PKG_LIST))

  # Defines for the app and cfg source code to know which components/packages
  # are included in the build for the local CORE...
  CFLAGS_APP_DEFINES = $(foreach PKG,$(PKG_LIST_C67X_LOCAL),-D_LOCAL_$(PKG)_)
  CFLAGS_APP_DEFINES += $(foreach PKG,$(PKG_LIST_C67X_LOCAL),-D_BUILD_$(PKG)_)

  ifeq ($(CORE),c674x)
    PKG_LIST_C67X_REMOTE = $(foreach COMP,$(COMP_LIST_c674x),$($(COMP)_PKG_LIST))
    CFLAGS_APP_DEFINES += -D_LOCAL_CORE_c674x_
  endif

  # Defines for the app and cfg source code to know which components/packages
  # are included in the build for the remote CORE...
  CFLAGS_APP_DEFINES += $(foreach PKG,$(PKG_LIST_C67X_REMOTE),-D_REMOTE_$(PKG)_)
  CFLAGS_APP_DEFINES += $(foreach PKG,$(PKG_LIST_C67X_REMOTE),-D_BUILD_$(PKG)_)
endif
...
LNKFLAGS_INTERNAL_COMMON = -w -q -u _c_int00 --silicon_version=6740 -c
