...
int main(void)
{
    ...
    UARTStdioInit();
    UARTPuts("\nTimer Application Running on C66x", -1);

    /* Run the Timer DSP1_IRQ_XBAR_COUNT times,
     * increment gXbarInst in each iteration */
    for (gXbarInst = 1; gXbarInst <= DSP1_IRQ_XBAR_COUNT; gXbarInst++)
    {
        TimerRun();
    }

    if (gNumSuccess == DSP1_IRQ_XBAR_COUNT)
    {
        UARTPuts("\nAll Xbar instances for C66x are verified successfully", -1);
    }
    return 0;
}
...
static void TimerIntcInit(void)
{
    irq_xbar_ret_type_t status = invalid_dsp_id;
    ...
    status = IRQXBARConnect(SOC_IRQ_DMARQ_CROSSBAR_REGISTERS_BASE, CPU_DSP1,
                            gXbarInst, TIMER4_IRQ);

    if (status == irq_xbar_success)
    {
        /* Check if xbar is indeed connected correctly */
        IRQXBARIsConnected(SOC_IRQ_DMARQ_CROSSBAR_REGISTERS_BASE, CPU_DSP1,
...
static void TimerIntcDeInit(void)
{
    ...
    /* Disconnect the XBar */
    IRQXBARDisconnect(SOC_IRQ_DMARQ_CROSSBAR_REGISTERS_BASE, CPU_DSP1, gXbarInst);
