...
int main(void)
{
    ...
    UARTStdioInit();
    UARTPuts("\nTimer Application Running on M4", -1);

    /* Run the Timer IPU1_IRQ_XBAR_COUNT times,
     * increment gXbarInst in each iteration */
    for (gXbarInst = 1; gXbarInst <= IPU1_IRQ_XBAR_COUNT; gXbarInst++)
    {
        TimerRun();
    }

    if (gNumSuccess == IPU1_IRQ_XBAR_COUNT)
    {
...
static void TimerIntcDeInit(void)
{
    ...
    /* Disconnect the XBar */
    IRQXBARDisconnect(SOC_IRQ_DMARQ_CROSSBAR_REGISTERS_BASE, CPU_IPU1, gXbarInst);
