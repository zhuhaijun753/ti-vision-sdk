                           ...
                           uint32_t nblks)
{
    ...
    ctrl->dmaEnable     = 1;
    ctrl->edmaChan      = ctrl->edmaTxDreq;
    edmaParam.baseAddr  = SOC_EDMA_TPCC_BASE;
    edmaParam.srcBfr    = (uint32_t) ptr + offset;
    edmaParam.dstBfr    = ctrl->memBase + MMC_DATA;
    edmaParam.chan      = ctrl->edmaChan;
    ...
    if (ctrl->dmaEnable == 1)
    {
        HSMMCSDEdmaWaitTransfer(ctrl->edmaChan);
    }
