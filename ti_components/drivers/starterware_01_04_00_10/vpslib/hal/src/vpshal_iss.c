...
#include <hw/hw_iss_sys1.h>
#include <hw/hw_iss_sys2.h>
#include <hw/hw_iss_sys3.h>
...
#ifdef VPS_ISS_BUILD
/* CAUTION - Since we cannot support capture via CAL B, initialize CAL A only */
static isshalCalInstParams_t      gIssHalCalInstParams
[CSL_ISS_CAL_PER_CNT] =
{{0, ISS_CALA_BASE_REGISTER, ISS_CALA_CAMERARX_CORE_0_BASE_REGISTER, NULL},
 ...
 * this variable.
 */
static const issemInitParams_t    gIssHalCommonPlatformData =
{
    1U,
    ...
    IEM_MODULE_ISS_MAX,
    {ISS_CALA_BASE_REGISTER,    ISS_TOP_BASE_REGISTER,
     ISS_ISP_SYS1_BASE_REGISTER,
...
int32_t VpsHal_issctrlInit(const isshalIssCtrlInitParams_t *initPrms, Ptr arg)
{
    ...
    if((uint32_t)FALSE == gIssHalIssCtrlCfg.isInitDone)
    {
        ...
        **/
        ...
        regVal &= ~ISS_CTRL_INPUT_SEL_MASK;
        regVal |= (ISS_CTRL_INPUT_SEL_PAR << ISS_CTRL_INPUT_SEL_SHIFT);
    ...
    if(BSP_SOK == rtnValue)
    {
        ...
        switch(srcPort)
        {
            case SRC_PORT_CAL_A_BYS_OUT:
                ...
                {
                    regVal &= ~ISS_VMUX_NSF3V_IN_MASK;
                    if(TRUE == isEnable)
                    {
                        regVal |= ISS_VMUX_NSF3V_IN_CAL_A_BYS_OUT <<
                                  ISS_VMUX_NSF3V_IN_SHIFT;
                    }
                }
                else
                {
                    regVal &= ~ISS_VMUX_GLBCE_IN_MASK;
                    if(TRUE == isEnable)
                    {
                        regVal |= ISS_VMUX_GLBCE_IN_CAL_A_BYS_OUT <<
                                  ISS_VMUX_GLBCE_IN_SHIFT;
                    }
                }
                break;
            case SRC_PORT_CAL_A_VP:
                regVal &= ~ISS_VMUX_ISP_IN_MASK;
                if(TRUE == isEnable)
                {
                    regVal |= ISS_VMUX_ISP_IN_CAL_A_OUT <<
            ...
            case SRC_PORT_CAL_B_BYS_OUT:
                if((sinkPort & SINK_PORT_NSFV3_IN) == SINK_PORT_NSFV3_IN)
                {
                    ...
                    if(TRUE == isEnable)
                    {
                        regVal |= ISS_VMUX_NSF3V_IN_CAL_B_BYS_OUT <<
                                  ISS_VMUX_NSF3V_IN_SHIFT;
                    }
                }
                else
                {
                    ...
                    if(TRUE == isEnable)
                    {
                        regVal |= ISS_VMUX_GLBCE_IN_CAL_B_BYS_OUT <<
                                  ISS_VMUX_GLBCE_IN_SHIFT;
                    }
                }
                break;
            case SRC_PORT_CAL_B_VP:
                ...
                if(TRUE == isEnable)
                {
                    regVal |= ISS_VMUX_ISP_IN_CAL_B_OUT <<
                              ISS_VMUX_ISP_IN_SHIFT;
                }
                break;
            case SRC_PORT_PARALLEL_A:
                regVal &= ~ISS_VMUX_ISP_IN_MASK;
                regVal |= ISS_VMUX_ISP_IN_PARALLEL_A <<
                          ISS_VMUX_ISP_IN_SHIFT;
                break;
            case SRC_PORT_NSF3V_OUT:
                if((sinkPort & SINK_PORT_CAL_A_BYS_IN) ==
                   SINK_PORT_CAL_A_BYS_IN)
                {
                    regVal &= ~ISS_VMUX_CAL_A_BYS_IN_MASK;
                    regVal |= ISS_VMUX_CAL_A_BYS_IN_NSF3V_OUT <<
                              ISS_VMUX_CAL_A_BYS_IN_SHIFT;
                }
                else
                {
                    regVal &= ~ISS_VMUX_CAL_B_BYS_IN_MASK;
                    regVal |= ISS_VMUX_CAL_A_BYS_IN_GLBCE_OUT <<
            ...
            case SRC_PORT_GLBCE_OUT:
                ...
                {
                    regVal &= ~ISS_VMUX_CAL_B_BYS_IN_MASK;
                    regVal |= ISS_VMUX_CAL_B_BYS_IN_GLBCE_OUT <<
                              ISS_VMUX_CAL_B_BYS_IN_SHIFT;
                }
                break;
            case SRC_PORT_LVDSRX_OUT_0:
                ...
                {
                    regVal &= ~ISS_VMUX_ISP_IN_MASK;
                    regVal |= ISS_VMUX_ISP_IN_LVDSRX_OUT0 <<
                              ISS_VMUX_ISP_IN_SHIFT;
                }
                ...
                {
                    regVal &= ~ISS_VMUX_CAL_A_BYS_IN_MASK;
                    regVal |= ISS_VMUX_CAL_A_BYS_IN_LVDSRX_OUT0 <<
                              ISS_VMUX_GLBCE_IN_SHIFT;
                }
                else
                {
                    regVal &= ~ISS_VMUX_CAL_B_BYS_IN_MASK;
                    regVal |= ISS_VMUX_CAL_B_BYS_IN_LVDSRX_OUT1 <<
                              ISS_VMUX_GLBCE_IN_SHIFT;
                }
                break;
            case SRC_PORT_LVDSRX_OUT_3:
                ...
                {
                    regVal &= ~ISS_VMUX_ISP_IN_MASK;
                    regVal |= ISS_VMUX_ISP_IN_LVDSRX_OUT3 <<
                              ISS_VMUX_CAL_A_BYS_IN_SHIFT;
                }
                else if((sinkPort & SINK_PORT_CAL_A_BYS_IN) ==
                        SINK_PORT_CAL_A_BYS_IN)
                {
                    regVal &= ~ISS_VMUX_CAL_A_BYS_IN_MASK;
                    regVal |= ISS_VMUX_CAL_A_BYS_IN_LVDSRX_OUT3 <<
                              ISS_VMUX_CAL_A_BYS_IN_SHIFT;
                }
                else
                {
                    regVal &= ~ISS_VMUX_CAL_B_BYS_IN_MASK;
                    regVal |= ISS_VMUX_CAL_B_BYS_IN_LDVSRX_OUT3 <<
                              ISS_VMUX_CAL_B_BYS_IN_SHIFT;
                }
                break;
            default:
...
Void VpsHal_issSetVbusMPriority(isshalIssVBusMPriority_t prio)
{
    ...
    regVal  = HW_RD_REG32(gIssHalIssCtrlCfg.ispSys1BaseAddr + ISP5_CTRL);
    regVal &= ~ISP5_CTRL_VBUSM_CPRIORITY_MASK;
...
Void VpsHal_issSetModuleBwCtrl(isshalIssBwCtrlModule_t module, uint32_t bwCtrl)
{
    ...
    switch(module)
    {
        ...
        case ISSHAL_ISS_BW_CTRL_MODULE_IPIPEIF_READ:
            regVal = HW_RD_REG32(
                gIssHalIssCtrlCfg.ispSys1BaseAddr + ISP5_BL_MTC_1);
            regVal &= ~ISP5_BL_MTC_1_IPIPEIF_R_MASK;
                              ...
                              isshalGlbceSwitchCtrl_t   ctrl)
{
    ...
    {
        regVal &= ~ISP_SWT_SEL_NSF3V_MASK;
        regVal |= (ctrl << ISP_SWT_SEL_NSF3V_SHIFT) &
                  ISP_SWT_SEL_NSF3V_MASK;
    }
    else /* GLBCE Module */
    {
        regVal &= ~ISP_SWT_SEL_GLBCE_MASK;
...
int32_t VpsHal_issSetVp1Config(const isshalissVp1Config_t *cfg)
{
    ...
    regVal &= ~ISP_VP1_LNH_MASK;
    regVal |= ((cfg->cropWin.cropWidth - 1U) << ISP_VP1_LNH_SHIFT) &
              ISP_VP1_LNH_MASK;
    ...
    regVal &= ~ISP_VP1_LNV_MASK;
    regVal |= ((cfg->cropWin.cropHeight - 1U) << ISP_VP1_LNV_SHIFT) &
              ISP_VP1_LNV_MASK;
    ...
    regVal &= ~ISP_NSF3V_MODESET_INPMOD_MASK;
