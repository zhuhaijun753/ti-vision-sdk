...
#include <hal/src/vpshal_scPriv.h>
#include <hal/src/vpshal_scDefaults.h>
    ...
    else
    {
        *offsetA = 0U; /* TODO: How to calculate offsetA */
        if (FVID2_SF_INTERLACED == advCfg->inFrameMode)
        {
            if (FVID2_SF_INTERLACED == advCfg->outFrameMode)
            {
                *rowAccInc = (((advCfg->cropHeight << 2U) - 1U) <<
                              SC_ROW_ACC_INC_SHIFT) /
                             ((advCfg->tarHeight << 2U) - 1U);
                *offsetB = (((advCfg->cropHeight - 1U) <<
            ...
            else /* Progressive output frame */
            {
                *rowAccInc = (((advCfg->cropHeight << 2U) - 1U) <<
                              SC_ROW_ACC_INC_SHIFT) /
                             ((advCfg->tarHeight - 1U) << 2U);
                *offsetB =
                    (UInt32) ((Int32) ((UInt32) 1U <<
                                       (SC_ROW_ACC_INC_SHIFT - 1U)) * (-1));
            }
        }
        else /* Progressive input frame */
        {
            if (FVID2_SF_INTERLACED == advCfg->outFrameMode)
            {
                *rowAccInc = ((advCfg->cropHeight - 1U) <<
                              (SC_ROW_ACC_INC_SHIFT + 1U)) /
                             ((advCfg->tarHeight << 2U) - 1U);
                *offsetB = ((advCfg->cropHeight - 1U) <<
                            SC_ROW_ACC_INC_SHIFT) /
                                             ...
                                             UInt32                *ravInit)
{
    ...
    {
        *factor = (UInt16) ((advCfg->tarHeight << SC_HORZ_SCALE_SHIFT) /
                            advCfg->cropHeight);
