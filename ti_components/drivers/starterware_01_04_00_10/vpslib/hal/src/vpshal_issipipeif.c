    ...
    uint32_t                              regOffset)
{
    ...
    if((uint32_t)TRUE == cfg->enable)
    {
        ...
        else
        {
            ...
            reg &= ~IPIPEIF_VPDCMPXTHR1_XTHR1_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPXTHR2_XTHR2_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPXTHR3_XTHR3_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPYTHR1_YTHR1_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPYTHR2_YTHR2_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPYTHR3_YTHR3_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPSLOPE1_SLOPE1_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPSLOPE2_SLOPE2_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPSLOPE3_SLOPE3_MASK;
            ...
            reg &= ~IPIPEIF_VPDCMPSLOPE4_SLOPE4_MASK;
    ...
    uint32_t                           regOffset)
{
    ...
    {
        reg  = HW_RD_REG32(baseAddr + regOffset + 0x0U);
        reg &= ~IPIPEIF_WDRSAT_VP_SAT_MASK;
        reg |= (IPIPEIF_WDRSAT_VP_SAT_MASK & cfg->sat);
        HW_WR_REG32(baseAddr + regOffset + 0x0U, reg);

        reg  = HW_RD_REG32(baseAddr + regOffset + 0x4U);
        reg &= ~(IPIPEIF_WDRSAT_VP2_VP_DSF_MASK |
                                   ...
                                   uint32_t                      regOffset)
{
    ...
    reg     &= ~(IPIPEIF_DPC1_TH_MASK | IPIPEIF_DPC1_ENA_MASK);
    if((uint32_t)TRUE == cfg->enable)
    {
                                    ...
                                    const isshalIpipeifDpcmCfg_t *cfg)
{
    ...
    reg &= ~(IPIPEIF_DPCM_ENA_MASK | IPIPEIF_DPCM_PRED_MASK |
                                       ...
                                       const isshalIpipeifIsifOutCfg_t *cfg)
{
    ...
    reg &= ~(IPIPEIF_CFG1_INPSRC1_MASK | IPIPEIF_CFG1_ONESHOT_MASK);
    reg |= IPIPEIF_CFG1_INPSRC1_MASK &
           ((cfg->inpSrc1 - 1U) << IPIPEIF_CFG1_INPSRC1_SHIFT);
    ...
    const isshalIpipeifIpipeOutCfg_t *cfg)
{
    ...
    reg  = HW_RD_REG32(instCfg->baseAddr + IPIPEIF_CFG1);
    reg &= ~(IPIPEIF_CFG1_INPSRC2_MASK | IPIPEIF_CFG1_AVGFILT_MASK |
             IPIPEIF_CFG1_DECIM_MASK);
    reg |= IPIPEIF_CFG1_INPSRC2_MASK &
           ((cfg->inpSrc2 - 1U) << IPIPEIF_CFG1_INPSRC2_SHIFT);
    ...
    const isshalIpipeifH3aOutCfg_t *cfg)
{
    ...
    if(0x0 != cfg->deciCfg.crop)
    {
        reg2 &= ~(IPIPEIF_INIRSZ3A_INIRSZ_MASK);
        reg2 |= IPIPEIF_INIRSZ3A_ALNSYNC_MASK;
                                  ...
                                  const isshalIpipeifRtCfg_t *cfg)
{
    ...
    reg  = HW_RD_REG32(baseAddr + IPIPEIF_CFG1);
    reg &= ~(IPIPEIF_CFG1_INPSRC1_MASK | IPIPEIF_CFG1_INPSRC2_MASK);
    reg |= IPIPEIF_CFG1_INPSRC1_MASK &
           ((cfg->inpSrc1 - 1U) << IPIPEIF_CFG1_INPSRC1_SHIFT);
    reg |= IPIPEIF_CFG1_INPSRC2_MASK &
           ((cfg->inpSrc2 - 1U) << IPIPEIF_CFG1_INPSRC2_SHIFT);
                                ...
                                const isshalIpipeifCfg_t *cfg)
{
    ...
    if(FVID2_SOK == rtnVal)
    {
        ...
        regVal &= ~(IPIPEIF_CFG1_DATASFT_MASK | IPIPEIF_CFG1_UNPACK_MASK |
                                   ...
                                   const isshalIpipeifClkCfg_t *cfg)
{
    ...
    if((NULL != cfg) && (NULL != hndl))
    {
        ...
        regVal &= ~(IPIPEIF_CFG2_HDPOL_MASK | IPIPEIF_CFG2_VDPOL_MASK);
