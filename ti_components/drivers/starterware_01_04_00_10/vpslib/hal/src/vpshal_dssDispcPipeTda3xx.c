...
{298, 409, 0, 298, -208, -100, 298, 0, 517, -256, -2048, -2048};
static const Vps_DssCscCoeff   gWbCscCoeff =
{66, 129, 25, 112, -94, -18, -38, -74, 112, 0, 2048, 2048};
    ...
    if (retVal == BSP_SOK)
    {
        ...
        if (wbConf->scEnable != 0)
        {
            ...
            halDssVidConfScalerEnable(handle, scEnable);
                 ...
                 (wbConf->outFmt.dataFormat == FVID2_DF_YUV420SP_UV))
        {
            halDssVidConfScalerEnable(handle,
                                      (DSS_VID_ATTRIBUTES_RESIZEENABLE_VRESIZE
                                       |
            ...
            halDssVidConfFir2(handle, wbConf->inWidth,
                 ...
                 ((wbConf->outFmt.dataFormat == FVID2_DF_YUV422I_YUYV) ||
                  (wbConf->outFmt.dataFormat == FVID2_DF_YUV422I_UYVY)))
        {
                            ...
                            const Vps_DssDispcVidVC1Config  *vc1Cfg)
{
    ...
    if (retVal == BSP_SOK)
    {
        ...
        if (vidConf->scEnable)
        {
            if (vidConf->inFmt.width != vidConf->outWidth)
            {
                scEnable |= DSS_VID_ATTRIBUTES_RESIZEENABLE_HRESIZE;
            }
            if (vidConf->inFmt.height != vidConf->outHeight)
            {
                scEnable |= DSS_VID_ATTRIBUTES_RESIZEENABLE_VRESIZE;
            }
                              ...
                              UInt32        enable)
{
    ...
    if (retVal == BSP_SOK)
    {
        ...
        regVal &= ~DSS_VID_ATTRIBUTES_ENABLE_MASK;
    ...
    if (handleId <= VPS_DSS_DISPC_PIPE_WB)
    {
        ...
        if (handleId == VPS_DSS_DISPC_PIPE_WB)
        {
            if ((fvid2Fmt == FVID2_DF_BGRA32_1010102) ||
                (fvid2Fmt == FVID2_DF_BGRA32_1010102) ||
                                        ...
                                        UInt32        pixelInc)
{
    ...
    if (dispcVidObj->halId != VPSHAL_DSS_DISPC_WB1)
    {
        ...
        regVal &= ~(DSS_VID_PIXEL_INC_PIXELINC_MASK);
                                        ...
                                        UInt32        arbitration)
{
    ...
    if (dispcVidObj->halId == VPSHAL_DSS_DISPC_GFX)
    {
        regVal &= ~DSS_GFX_ATTRIBUTES_ARBITRATION_MASK;
        regVal |=
    ...
    {
        regVal &= ~DSS_VID_ATTRIBUTES_ARBITRATION_MASK;
        regVal |=
                                        ...
                                        UInt32        selfRefreshEnable)
{
    ...
    if (dispcVidObj->halId == VPSHAL_DSS_DISPC_GFX)
    {
        regVal &= ~DSS_GFX_ATTRIBUTES_SELFREFRESH_MASK;
        regVal |=
            (((UInt32) selfRefreshEnable) <<
    ...
    {
        regVal &= ~DSS_VID_ATTRIBUTES_SELFREFRESH_MASK;
        regVal |=
            (((UInt32) selfRefreshEnable) <<
                               ...
                               UInt32        dataFmt)
{
    ...
    regVal &= ~DSS_VID_ATTRIBUTES_FORMAT_MASK;
                                      ...
                                      UInt32        cscFullRngEnable)
{
    ...
    if ((dispcVidObj->halId == VPSHAL_DSS_DISPC_VID1) ||
        (dispcVidObj->halId == VPSHAL_DSS_DISPC_VID2))
    {
        regVal &= ~DSS_VID_ATTRIBUTES_FULLRANGE_MASK;
        regVal |=
            (((UInt32) cscFullRngEnable) <<
    ...
    {
        regVal &= ~DSS_WB_ATTRIBUTES_FULLRANGE_MASK;
        regVal |=
            (((UInt32) cscFullRngEnable) <<
                                  ...
                                  const Vps_DssCscCoeff *coeff)
{
        ...
        (dispcVidObj->halId == VPSHAL_DSS_DISPC_WB1))
    {
        ...
        regVal &= ~(DSS_VID_CONV_COEF4_BCB_MASK);
        ...
        regVal &= ~(DSS_VID_CONV_COEF6_BOFFSET_MASK);
    ...
    {
        /* Check if same offsets(for VID) work for WB as well */
    }
    else
    {
        /* Error */
                                   ...
                                   UInt32        cscEnable)
{
    ...
    if ((dispcVidObj->halId == VPSHAL_DSS_DISPC_VID1) ||
        (dispcVidObj->halId == VPSHAL_DSS_DISPC_VID2))
    {
        regVal &= ~DSS_VID_ATTRIBUTES_COLORCONVENABLE_MASK;
        regVal |=
            (((UInt32) cscEnable) <<
    ...
    if (dispcVidObj->halId == VPSHAL_DSS_DISPC_WB1)
    {
        regVal &= ~DSS_WB_ATTRIBUTES_COLORCONVENABLE_MASK;
        regVal |=
                                    ...
                                    const VpsHal_DssScCoeff *hChromaCoef)
{
    ...
    if (dispcVidObj->halId != VPSHAL_DSS_DISPC_GFX)
    {
        for (i = 0U; i < 9U; i++)
        {
            ...
            regVal &= ~DSS_VID_FIR_COEF_H0_FIRHC0_MASK;
            regVal |=
            ...
            regVal &= ~DSS_VID_FIR_COEF_H0_C_FIRHC0_MASK;
            regVal |=
            ...
            regVal &= ~DSS_VID_FIR_COEF_V0_FIRVC0_MASK;
            regVal |=
            ...
            regVal &= ~DSS_VID_FIR_COEF_V0_C_FIRVC0_MASK;
            regVal |=
                                      ...
                                      UInt32        scalerEnable)
{
    ...
    regVal &= ~DSS_VID_ATTRIBUTES_RESIZEENABLE_MASK;
    regVal |=
                               ...
                               UInt32        vc1Enable)
{
        ...
        (dispcVidObj->halId == VPSHAL_DSS_DISPC_VID2))
    {
        ...
        regVal &= ~(DSS_VID_ATTRIBUTES2_VC1ENABLE_MASK);
                               ...
                               UInt32        outWidth)
{
    ...
    else
    {
        ...
        regVal &= ~DSS_WB_REGION_BASED_TOTAL_PICTURE_SIZEY_MASK;
                              ...
                              UInt32        inSizeX)
{
    ...
    {
        regVal  = HW_RD_REG32(dispcVidObj->regOffset.pictSize);
        regVal &= ~DSS_VID_PICTURE_SIZE_MEMSIZEX_MASK;
        regVal |=
            (((UInt32) (inSizeX -
                        1U)) << DSS_VID_PICTURE_SIZE_MEMSIZEX_SHIFT) &
            DSS_VID_PICTURE_SIZE_MEMSIZEX_MASK;

        regVal &= ~DSS_VID_PICTURE_SIZE_MEMSIZEY_MASK;
        regVal |=
            (((UInt32) (inSizeY -
                                           ...
                                           UInt32        antiFlickerEnable)
{
    ...
    if (dispcVidObj->halId == VPSHAL_DSS_DISPC_GFX)
    {
        ...
        regVal &= ~DSS_GFX_ATTRIBUTES_ANTIFLICKER_MASK;
        regVal |=
            (((UInt32) antiFlickerEnable) <<
                                    ...
                                    UInt32        nibblemode)
{
    ...
    regVal &= ~DSS_VID_ATTRIBUTES_NIBBLEMODE_MASK;
