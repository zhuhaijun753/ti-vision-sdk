...
#include <soc.h>
#include <hw_types.h>
#include <hw_hdmi_phy.h>
#include <hw_hdmi_wp.h>
#include <hw_hdmi_pll.h>
#include <hw_hdmi_dwc_tx.h>
...
static Int32 halHdmiPllInit(const VpsHal_DssHdmiPllInfo *pllInfo)
{
    ...
    if (retVal == BSP_SOK)
    {
        ...
        while ((HW_RD_FIELD32(
                    ...
                    HDMI_PLL_STATUS_LOCK) != HDMI_PLL_STATUS_LOCK_DSI_LOCK) &&
               (cnt < 10U))
        {
            BspOsal_sleep((UInt32) 1U);
            cnt++;
...
static Int32 halHdmiWpSetPhyPwr(vpsHal_hdmiPhyPwr status)
{
    ...
    while ((HW_RD_FIELD32(SOC_HDMI_WP_BASE + HDMI_WP_PWR_CTRL,
                          HDMI_WP_PWR_CTRL_PHY_STATUS) != status)
           && (cnt < 10U))
    {
        BspOsal_sleep((UInt32) 1U);
        cnt++;
...
void VpsHal_hdmiBasicConfigure(const VpsHal_DssHdmiTimingParam *hdmiCfg)
{
    ...
    {
        if ((hdmiCfg->mode == VPSHAL_HDMI_DVI) ||
            ((HdmiHalInfo.advParam.standardCode == 1) &&
             (hdmiCfg->mode == VPSHAL_HDMI_HDMI)))
        {
