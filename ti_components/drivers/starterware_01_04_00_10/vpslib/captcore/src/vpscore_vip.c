...
#define VPS_VIP_DEF_DESC_SIZE (1024U)
/*
 * Max number of descriptors allocated per pipe object
 * num pathObj->descMem = VPS_CFG_MEM_VIP_MAX_PORT_OBJS
 * num pathObj->wrDescMem = VIP_SW_Q_LEN * VPS_CFG_MEM_VIP_MAX_PORT_OBJS
 * num pathObj->cfgDescMemSize = 1
 * num pathObj->ctrlDescMem = 1 */
#define VPS_VIP_MAX_DESC_PER_CH (VPS_CFG_MEM_VIP_MAX_PORT_OBJS + \
                                 (VIP_SW_Q_LEN *                 \
                                  VPS_CFG_MEM_VIP_MAX_PORT_OBJS) + 1U + 1U)
 ...
 *  allocated or not
 */
static UInt32              gVcoreVipPathObjMemFlag[
    VPS_CFG_MEM_VIP_MAX_PORT_OBJS];
...
#else
#pragma DATA_ALIGN(gVcoreVipDescMemPool, VPSHAL_VPDMA_DESC_BYTE_ALIGN);
#pragma DATA_SECTION(gVcoreVipDescMemPool,".bss:extMemNonCache:vpdma");
#endif  /* #ifdef __cplusplus */
static UInt8 gVcoreVipDescMemPool
[VPS_VIP_MAX_DESC_PER_CH * VPS_CFG_MEM_VIP_MAX_PORT_OBJS]
[VPS_VIP_DEF_DESC_SIZE];
/** \brief Pool params for descriptor memory. */
static BspUtils_PoolParams gVcoreVipDescMemPoolPrm;
/** \brief The flag variable represents whether a handle object is allocated
 *   or not. */
static UInt32              gVcoreVipDescMemFlag
[VPS_VIP_MAX_DESC_PER_CH * VPS_CFG_MEM_VIP_MAX_PORT_OBJS];
...
VpsCore_Inst VpsCore_vipInit(const VpsCore_VipInitParams *initParams)
{
    ...
    if (FALSE == gVpsCorePoolObjInitDone)
    {
        ...
        BspUtils_initPool(&gVcoreVipPathObjPoolParams,
                          ...
                          VpsVipCoreTrace);
        BspUtils_initPool(
            &gVcoreVipDescMemPoolPrm,
            (Void *) gVcoreVipDescMemPool,
            (VPS_VIP_MAX_DESC_PER_CH * VPS_CFG_MEM_VIP_MAX_PORT_OBJS),
            VPS_VIP_DEF_DESC_SIZE,
            gVcoreVipDescMemFlag,
            VpsVipCoreTrace);
                                ...
                                Vgraph_edgeInfoList       *edgeInfoList)
{
    ...
    UInt32           cscSrcNode, cscNode, scSrcNode, scNode;
    UInt32           chrds0SrcOnNode, chrds0SrcOffNode;
    UInt32           chrds1SrcOnNode, chrds1SrcOffNode;
    UInt32           chrds0Node, chrds1Node;
    ...
    Bool             allowDirect422Path;
    UInt32           slice;
    Bool             scReqd;
    Bool             cscReqd;
    Bool             cscAvl;
    Bool             scAvl;
    Bool             chrds0Avl, chrds1Avl;
    Bool             chrdsReqd;
    ...
    for (i = 0; ((i < pathObj->numStreams) && (BSP_SOK == retVal)); i++)
    {
        scReqd  = (Bool) (1 == pathObj->scActive[i]);
        cscReqd = VpsCore_VipCscReqd(pathObj, i);
    ...
    else
    {
        ...
        while ((i < pathObj->numStreams) && (BSP_SOK == retVal))
        {
            ...
            if ((!gotoNextIteration) && (!dataNode24Bit) &&
                allowDirect422Path && (!scReqd) && (!cscReqd) &&
                (!spActive))
            {
            ...
            if ((!gotoNextIteration) && ((Bool) (BSP_SOK == retVal)) &&
                ((Bool) (is24BitOut && cscReqd)))
            {
            ...
            else if ((!gotoNextIteration) && ((Bool) (BSP_SOK == retVal)) &&
                     ((Bool) (is24BitOut && (!cscReqd))))
            {
                if ((yLoAvl) && (!yLoAdded) &&
                    (uvLoAvl) && (!uvLoAdded))
                ...
                else if (cscAvl && (!cscAdded) && (!rgbHiNodeAdded) &&
                         (yUpAvl) && (!yUpAdded) &&
                         (uvUpAvl) && (!uvUpAdded))
                {
             ...
             */
            if ((!gotoNextIteration) && ((Bool) (BSP_SOK == retVal)) &&
                ((scReqd || cscReqd || dataChrdsForced) && (!is24BitOut)))
            {
                if (cscReqd && scReqd)
                {
                    startNode = scNode;
                ...
                if (chrds0Avl && (!chrds0Added) && tmp)
                {
                    ...
                    if (!chrds0Added)
                    {
                        ...
                        tmp         = Vgraph_isNodeInputAvailable(
                            nodeList,
                            chrds1SrcOffNode);
                        if ((!yUpAvl) || yUpAdded || rgbHiNodeAdded)
                        {
                            chrds0Added = FALSE;
                        }
                        else if ((spActive) &&
                ...
                if ((!gotoNextIteration) && chrds1Avl && tmp)
                {
                    /* Connect a node to CHR_DS only if it is not connected to
                     * something else already */
                         ...
                         (!(spActive && uvLoAdded))))
                    {
                        ...
                        else if ((!chrds1outUpUsable) && chrds1outLoAdded)
                        {
                            chrds1Added = FALSE;
                        }
                              ...
                              const VpsCore_VipParams *pathCfg)
{
    ...
    if (BSP_SOK == retVal)
    {
                                         ...
                                         VGRAPH_NODE_CHECK))
        {
            ...
            /* TODO: Fix arbitrary size requirement */
            pathObj->descMemSize   = VPS_VIP_DEF_DESC_SIZE;
            pathObj->wrDescMemSize = pathObj->descMemSize;
            ...
            pathObj->cfgDescMemSize = VPS_VIP_DEF_DESC_SIZE;
             ...
             */
            pathObj->ctrlDescMemSize = VPS_VIP_DEF_DESC_SIZE;
...
Void VpsCore_vipPathResFree(VpsCore_VipPathObj *pathObj)
{
    ...
    for (chId = 0; chId < pathObj->numChannels; chId++)
    {
        ...
        {
            BspUtils_free(&gVcoreVipDescMemPoolPrm,
    ...
    {
        BspUtils_free(&gVcoreVipDescMemPoolPrm, (void *) pathObj->cfgDescMem,
                      VpsVipCoreTrace);
        pathObj->cfgDescMem = NULL;
    }
    ...
    {
        BspUtils_free(&gVcoreVipDescMemPoolPrm, (void *) pathObj->ctrlDescMem,
                      VpsVipCoreTrace);
        pathObj->ctrlDescMem = NULL;
    }
}
                                     ...
                                     const Vgraph_nodeInfo    *curNode)
{
    ...
    if (0 != curNode->input.isEnabled[1])
    {
    ...
    else if (0 != curNode->input.isEnabled[0])
    {
        if (!chrDs0Bypass)
        {
                                      ...
                                      const Vgraph_nodeInfo    *curNode)
{
    ...
    if (0 != curNode->input.isEnabled[0])
    {
    ...
    else if (0 != curNode->input.isEnabled[3])
    {
                                     ...
                                     const Vgraph_nodeInfo    *curNode)
{
    ...
    else if (0 != curNode->input.isEnabled[1])
    {
        portDataType[PORT_Y_LO] = VCORE_VIPVPDMA_DT_YC422;
    }
    else if (0 != curNode->input.isEnabled[2])
    {
        Bool rgbSrc = (Bool) Fvid2_isDataFmtRgb(srcColorSpace);
        ...
        else if (((Bool) (1U == pathObj->cscActive)) && (!rgbSrc))
        {
...
void VpsCore_vipCreateCCISCtrlDesc(VpsCore_VipPathObj *pathObj)
{
    ...
    for (chId = 0; chId < pathObj->numChannels; chId++)
    {
        ...
        for (strm = 0; strm < numStreams; strm++)
        {
            ...
            descMem  += VPSHAL_VPDMA_CTRL_DESC_SIZE;
