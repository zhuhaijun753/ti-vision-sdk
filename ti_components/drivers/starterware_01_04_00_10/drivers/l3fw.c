                      ...
                      uint32_t portNum)
{
    ...
    /* Ignoring Region 0 as it can not be enabled */
    if (0U != regionId)
    {
        L3FWBlockTransactions(baseAddr);
                       ...
                       uint32_t portNum)
{
    ...
    /* Ignoring Region 0 as it can not be disabled */
    if (0U != regionId)
    {
        L3FWBlockTransactions(baseAddr);
        regVal |= ~((uint32_t) 0x1U << portNum);
                      ...
                      l3fwRegionErrorStatus_t *errorInfo)
{
    ...
    regVal = HW_RD_FIELD32(baseAddr + L3FW_ERROR_LOG(portNum),
                           L3FW_ERROR_LOG_REQINFO_ERRLOG);
    errorInfo->errorStartRegion = HW_RD_FIELD32(baseAddr +
                                                L3FW_ERROR_LOG(
                                                    portNum),
                                                L3FW_ERROR_LOG_REGION_START_ERRLOG);
    errorInfo->errorEndRegion = HW_RD_FIELD32(baseAddr +
                                              L3FW_ERROR_LOG(portNum),
                                              L3FW_ERROR_LOG_REGION_END_ERRLOG);
    errorInfo->blockBurstViolation = HW_RD_FIELD32(baseAddr +
                                                   L3FW_ERROR_LOG(
                                                       portNum),
                                                   L3FW_ERROR_LOG_BLK_BURST_VIOLATION);
    errorInfo->addrOffset = HW_RD_FIELD32(baseAddr +
                                          L3FW_LOGICAL_ADDR_ERRLOG(
                                              portNum),
                                          L3FW_LOGICAL_ADDR_ERRLOG_SLVOFS_LOGICAL);
    errorInfo->mReqType = (regVal & L3FW_ERROR_LOG_MReqType_MASK) >>
                          L3FW_ERROR_LOG_MReqType_SHIFT;
    errorInfo->mReqSupervisor = (regVal & L3FW_ERROR_LOG_SUPERVISOR_MASK) >>
                                L3FW_ERROR_LOG_SUPERVISOR_SHIFT;
    errorInfo->mReqDebug = (regVal & L3FW_ERROR_LOG_MReqDebug_MASK) >>
                           L3FW_ERROR_LOG_MReqDebug_SHIFT;
    errorInfo->accessType = (regVal & L3FW_ERROR_LOG_MCMD_MASK) >>
                            L3FW_ERROR_LOG_MCMD_SHIFT;
    errorInfo->connIdMSB4bit = (regVal & L3FW_ERROR_LOG_CONNID_MASK) >>
