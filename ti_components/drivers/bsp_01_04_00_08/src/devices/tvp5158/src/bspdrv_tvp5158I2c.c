                           ...
                           Bsp_VidDecChipIdStatus       *pStatus)
{
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        regAddr[numRegs]  = BSP_TVP5158_REG_CHIP_ID_MSB;
        regValue[numRegs] = 0;
        numRegs++;

        regAddr[numRegs]  = BSP_TVP5158_REG_CHIP_ID_LSB;
        regValue[numRegs] = 0;
        numRegs++;

        regAddr[numRegs]  = BSP_TVP5158_REG_ROM_VERSION;
        regValue[numRegs] = 0;
        numRegs++;

        regAddr[numRegs]  = BSP_TVP5158_REG_RAM_VERSION_0;
        regValue[numRegs] = 0;
        numRegs++;

        regAddr[numRegs]  = BSP_TVP5158_REG_RAM_VERSION_1;
        regValue[numRegs] = 0;
        numRegs++;
                              ...
                              const Bsp_VidDecVideoModeParams *pPrm)
{
    ...
    UInt8  regValue[9];
    UInt32 numRegs;
    UInt32 interleaveMode, chMuxNum, vidResSel, outputType, delay;
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        for(devCnt = 0; devCnt < pCreatePrms->numDevicesAtPort; devCnt++)
        {
            ...
            if(retVal == FVID2_SOK)
            {
                ...
                regAddr[numRegs]  = BSP_TVP5158_REG_AVD_OUT_CTRL_1;
                regValue[numRegs] =
                    (interleaveMode << 6) |
                    (chMuxNum << 4) |
                    (outputType << 3) |
                ...
                regAddr[numRegs]  = BSP_TVP5158_REG_AVD_OUT_CTRL_2;
                regValue[numRegs] =
                    ((videoCropEnable & 0x1U) << 6) |
                ...
                regAddr[numRegs] = BSP_TVP5158_REG_OFM_MODE_CTRL;
                /* 1: OSC out enabled  */
                regValue[numRegs] = ((UInt32) 1 << 5);
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_AUTO_SW_MASK;
                                ...
                                Bsp_VidDecVideoStatus             *pStatus)
{
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        regAddr[numRegs]  = BSP_TVP5158_REG_DEC_RD_EN;
        regVal            = 1U << chId;
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        regAddr[numRegs]  = BSP_TVP5158_REG_STATUS_1;
        regValue[numRegs] = 0;
        numRegs++;

        regAddr[numRegs]  = BSP_TVP5158_REG_STATUS_2;
        regValue[numRegs] = 0;
        numRegs++;

        regAddr[numRegs]  = BSP_TVP5158_REG_VID_STD_STATUS;
        regValue[numRegs] = 0;
        numRegs++;
...
Int32 Bsp_tvp5158Stop(Bsp_Tvp5158HandleObj *pObj)
{
    ...
    retVal = Bsp_tvp5158OutputEnable(pObj, FALSE);
    if(FVID2_SOK != retVal)
    {
                               ...
                               const Bsp_VidDecColorParams *pPrm)
{
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        /* set core ID */
        regAddr[numRegs]  = BSP_TVP5158_REG_DEC_WR_EN;
        regValue[numRegs] = chId;
        numRegs++;

        /* set video processing parameters */
        regAddr[numRegs] = BSP_TVP5158_REG_Y_BRIGHTNESS;
        ...
        regAddr[numRegs] = BSP_TVP5158_REG_Y_CONTRAST;
        ...
        regAddr[numRegs] = BSP_TVP5158_REG_C_SATURATION;
        ...
        regAddr[numRegs] = BSP_TVP5158_REG_C_HUE;
                            ...
                            const Bsp_Tvp5158VideoNfParams *pPrm)
{
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        regAddr[numRegs]  = BSP_TVP5158_REG_NR_MAX_NOISE;
        regValue[numRegs] = pPrm->maxNoise;
        numRegs++;

        regAddr[numRegs] = BSP_TVP5158_REG_NR_CTRL;

        flag       = (!(Bool) (pPrm->nfEnable));
        blkWidthUV = 1U;
        ...
        regVal =
            ((((UInt32) flag & 0x1U) << 0) |
             ((pPrm->colorKillerEnable & 0x1U) << (UInt32) 4U) |
             (blkWidthUV << 3U) |
             ((UInt32)0 << 2U));
                              ...
                              const Bsp_Tvp5158AudioModeParams *pPrm)
{
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        regAddr[numRegs]  = BSP_TVP5158_REG_AUDIO_SAMPLE_HZ;
        regValue[numRegs] = BSP_TVP5158_AUDIO_16KHZ;
        if(pPrm->samplingHz == 8000)
        {
            regValue[numRegs] = BSP_TVP5158_AUDIO_8KHZ;
        }
        ...
        regAddr[numRegs] = BSP_TVP5158_REG_AUDIO_MIXER;
        switch(pPrm->tdmChannelNum)
        {
            case 0:
                regValue[numRegs] = BSP_TVP5158_AUDIO_TDM_2CH;
                break;
            case 1:
                regValue[numRegs] = BSP_TVP5158_AUDIO_TDM_4CH;
                break;
            case 2:
                regValue[numRegs] = BSP_TVP5158_AUDIO_TDM_8CH;
                break;
            case 3:
                regValue[numRegs] = BSP_TVP5158_AUDIO_TDM_12CH;
                break;
            case 4:
                regValue[numRegs] = BSP_TVP5158_AUDIO_TDM_16CH;
                break;
            default:
        ...
        regAddr[numRegs]  = BSP_TVP5158_REG_AUDIO_CASCADE;
        regValue[numRegs] = (pPrm->cascadeStage & 0x03U);
        numRegs++;

        /* Added check for tdmChannelNum also, along with cascade stage */
        regAddr[numRegs] = BSP_TVP5158_REG_AUDIO_CTRL;
...
static Int32 Bsp_tvp5158OfmReset(Bsp_Tvp5158HandleObj *pObj)
{
    ...
    for(devCnt = 0; devCnt < pCreatePrms->numDevicesAtPort; devCnt++)
    {
        ...
        else
        {
            ...
            regAddr[numRegs]  = BSP_TVP5158_REG_MISC_OFM_CTRL;
...
static Int32 Bsp_tvp5158PatchDownload(Bsp_Tvp5158HandleObj *pObj, UInt32 devId)
{
    ...
    patchAddr = (UInt8 *) gBspTvp5158Patch;
...
static Int32 Bsp_tvp5158OutputEnable(Bsp_Tvp5158HandleObj *pObj, UInt32 enable)
{
    ...
    for(devCnt = 0; devCnt < pCreatePrms->numDevicesAtPort; devCnt++)
    {
        ...
        if(retVal == FVID2_SOK)
        {
            ...
            if(enable)
            {
                regValue[0] |= BSP_TVP5158_OUT_ENABLE;
            }
            else
            {
                regValue[0] &= ~BSP_TVP5158_OUT_ENABLE;
    ...
    if(retVal == FVID2_SOK)
    {
        ...
        for(devCnt = 0; devCnt < pCreatePrms->numDevicesAtPort; devCnt++)
        {
            ...
            if(retVal == FVID2_SOK)
            {
                ...
                regAddr[numRegs]  = BSP_TVP5158_REG_VID_STD_SELECT;
                regValue[numRegs] = autoSwitchMode;
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_OP_MODE_CTRL;
                /* New V-Bit control algorithm (number of active lines per frame
                 * is constant as total LPF varies) */
                regValue[numRegs] = 0x08;
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_FV_DEC_CTRL;
                /* Adaptive BOP/EOP control for TI816x enabled and F and V bits
                 * decoded from line count */
                regValue[numRegs] = 0x03;
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_FV_CTRL;
                /* V-PLL fast-lock disabled and windowed VSYNC pipe disabled */
                regValue[numRegs] = 0x06;
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_FBIT_DURATION;
                /* Maximum field duration set to 788 for NTSC and 938 for PAL */
                regValue[numRegs] = 0x60;
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_ESYNC_OFFSET_1;
                ...
                regAddr[numRegs] = BSP_TVP5158_REG_ESYNC_OFFSET_2;
                /* Default V-bit position relative to F-bit */
                regValue[numRegs] = 0x00;
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_MIN_F1_ACT;
                /* Minimum F1_to_active set to 8 lines */
                regValue[numRegs] = 0x08;
                numRegs++;

                regAddr[numRegs] = BSP_TVP5158_REG_Y_CTRL_2;
            ...
            if(retVal == FVID2_SOK)
            {
                ...
                regAddr[numRegs]  = BSP_TVP5158_REG_Y_CTRL_1;
                regValue[numRegs] = 0;
                numRegs++;
                ...
                regAddr[numRegs]  = BSP_TVP5158_REG_C_CTRL_1;
                regValue[numRegs] = 0;
                numRegs++;

                regAddr[numRegs]  = BSP_TVP5158_REG_C_CTRL_2;
                regValue[numRegs] = 0;
                numRegs++;
    ...
    if(FVID2_SOK == retVal)
    {
        ...
        while(numDev--)
        {
            ...
            if(retVal == FVID2_SOK)
            {
                ...
                regAddr[numRegs]  = BSP_TVP5158_REG_AUDIO_GAIN_1;
                regValue[numRegs] = 0;
                numRegs++;

                regAddr[numRegs]  = BSP_TVP5158_REG_AUDIO_GAIN_2;
                regValue[numRegs] = 0;
                numRegs++;
