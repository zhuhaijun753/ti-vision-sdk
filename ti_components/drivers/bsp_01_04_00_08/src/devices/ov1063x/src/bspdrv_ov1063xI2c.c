                                 ...
                                 UInt32                      bStartSensor)
{
    ...
    if (status == FVID2_SOK)
    {
        ...
        nRegValue[0] = ((BSP_OV1063X_OPERATING_MODE_REG) & 0xFFFFU) >> 8;
        /* LSB of address 0x0100 */
        nRegValue[1] = ((BSP_OV1063X_OPERATING_MODE_REG) & 0x00FFU);
...
Int32 Bsp_ov1063xSoftwareReset(const Bsp_Ov1063xHandleObj *pObj)
{
    ...
    if (status == FVID2_SOK)
    {
        ...
        nRegValue[0] = ((BSP_OV1063X_SOFTWARE_RESET_REG) & 0xFFFFU) >> 8;
        /* LSB of address 0x0103 */
        nRegValue[1] = ((BSP_OV1063X_SOFTWARE_RESET_REG) & 0x00FFU);
        /* Reset value 0x01 */
        nRegValue[2] = 0x01;
        numRegs      = 3U;
        status       = Bsp_deviceRawWrite8(nI2cInstId,
    ...
    numSensorConfigEntries)
{
    ...
    if (FVID2_SOK == status)
    {
        for (nIndex = 0; nIndex < nArraySize; nIndex++)
        {
            ...
            /* MSB of the address */
            nRegValue[0] = (nAddr & 0xFFFFU) >> 8;
            /* LSB of the address */
            nRegValue[1] = (nAddr & 0x00FFU);
            /* Data */
