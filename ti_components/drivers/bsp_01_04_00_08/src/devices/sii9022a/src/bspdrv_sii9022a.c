...
static Bsp_Sii9022aModeInfo  gBspSii9022aModeInfo[] =
{
    ...
    {FVID2_STD_576P,     18,  2700,  50,864,   625,
      {12,                 0,   64,    5,
      5}, {132,          44,  720,   576}},
    {FVID2_STD_XGA_60,   2,   6500,  60,1344,  806,
      {24,                 0,  136,    3,
      6}, {296,          35,  1024,  768}},
    ...
    const Bsp_Sii9022aModeInfo *modeInfo)
{
    ...
    regValue[numRegs] = modeInfo->extSyncPrms.deDelay & 0xFFU;
    numRegs++;

    regAddr[numRegs]       = 0x63U;
    pObj->syncPolarityReg &= ~(0x03U);
    pObj->syncPolarityReg |=
        ((modeInfo->extSyncPrms.deDelay & 0x300U) >> 8);
    regValue[numRegs] = pObj->syncPolarityReg & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x64U;
    regValue[numRegs] = modeInfo->extSyncPrms.deTop & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x66U;
    regValue[numRegs] = modeInfo->extSyncPrms.deCnt & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x67U;
    regValue[numRegs] = (modeInfo->extSyncPrms.deCnt & 0xF00U) >> 8;
    numRegs++;

    regAddr[numRegs]  = 0x68U;
    regValue[numRegs] = modeInfo->extSyncPrms.deLine & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x69U;
    regValue[numRegs] = (modeInfo->extSyncPrms.deLine & 0x700U) >> 8;
    numRegs++;

    regAddr[numRegs]  = 0x00U;
    regValue[numRegs] = modeInfo->pixClk & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x01U;
    regValue[numRegs] = (modeInfo->pixClk & 0xFF00U) >> 8;
    numRegs++;

    regAddr[numRegs]  = 0x02U;
    regValue[numRegs] = modeInfo->vFreq & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x03U;
    regValue[numRegs] = (modeInfo->vFreq & 0xFF00U) >> 8;
    numRegs++;

    regAddr[numRegs]  = 0x04U;
    regValue[numRegs] = modeInfo->pixels & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x05U;
    regValue[numRegs] = (modeInfo->pixels & 0xFF00U) >> 8;
    numRegs++;

    regAddr[numRegs]  = 0x06U;
    regValue[numRegs] = modeInfo->lines & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x07U;
    regValue[numRegs] = (modeInfo->lines & 0xFF00U) >> 8;
    numRegs++;
    ...
    const Bsp_Sii9022aModeInfo *modeInfo)
{
    ...
    numRegs           = 0;
    regAddr[numRegs]  = 0x62U;
    regValue[numRegs] = modeInfo->embSyncPrms.hBitToHSync & 0xFFU;
    numRegs++;
    ...
    regAddr[numRegs]  = 0x64U;
    regValue[numRegs] = modeInfo->embSyncPrms.field2Offset & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x65U;
    regValue[numRegs] = (modeInfo->embSyncPrms.field2Offset & 0xF00U) >> 8;
    numRegs++;

    regAddr[numRegs]  = 0x66U;
    regValue[numRegs] = modeInfo->embSyncPrms.hWidth & 0xFFU;
    numRegs++;

    regAddr[numRegs]  = 0x67U;
    regValue[numRegs] = (modeInfo->embSyncPrms.hWidth & 0x300U) >> 8;
    numRegs++;

    regAddr[numRegs]  = 0x68U;
    regValue[numRegs] = modeInfo->embSyncPrms.vBitToVSync & 0x3FU;
    numRegs++;

    regAddr[numRegs]  = 0x69U;
    regValue[numRegs] = modeInfo->embSyncPrms.vWidth & 0x3FU;
    numRegs++;
    ...
    const Bsp_Sii9022aModeInfo  *modeInfo)
{
    ...
    regValue[regCnt] = modeInfo->modeCode & 0x7FU;
    regCnt++;
