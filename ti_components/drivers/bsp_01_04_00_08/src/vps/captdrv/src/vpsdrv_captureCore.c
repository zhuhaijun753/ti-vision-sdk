                                 ...
                                 Vps_CaptVipParams  *vipPrms)
{
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        vipPrms->scPrms->enableCoeffLoad = FALSE; /* Not supported currently */
    }
...
static Int32 vpsDrvCaptOpenIssCore(VpsDrv_CaptInstObj *instObj)
{
    ...
    issCaptCoreOpenPrms.csi2PhyClock = issSpecificOpenParms->csi2PhyClock;
    ...
    for (index = 0; index < VPS_ISS_CAL_MAX_CMPLXIO_INST; index++)
    {
        if (TRUE == issSpecificOpenParms->isCmplxIoCfgValid)
        {
            BspUtils_memcpy(
    ...
    issCaptCoreOpenPrms.isCmplxIoCfgValid =
        issSpecificOpenParms->isCmplxIoCfgValid;
