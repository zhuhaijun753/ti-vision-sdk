                     ...
                     const VpsMdrv_VpeInitParams *initPrms)
{
    ...
    for (instCnt = 0U; instCnt < numInst; instCnt++)
    {
        ...
        if (TRUE == instObj->isCoreReq[VPSMDRV_VPE_VPE_CORE_IDX])
        {
        ...
        if (TRUE == instObj->isCoreReq[VPSMDRV_VPE_VWB_CORE_IDX])
        {
        ...
        for (scalerId = 0U; scalerId < VPS_M2M_SCALER_ID_MAX; scalerId++)
        {
             ...
             * here. */
            instObj->lazyLoadingEnable[scalerId] = FALSE;
                            ...
                            VpsMdrv_VpeChObj     *chObj)
{
    ...
    if (FVID2_SOK == retVal)
    {
        for (descSet = 0U; descSet < VPSMDRV_VPE_MAX_DESC_SET; descSet++)
        {
            ...
            GT_assert(VpsMdrvVpeTrace,
                      (TRUE == instObj->isCoreReq[VPSMDRV_VPE_VPE_CORE_IDX]));
                                      ...
                                      VpsMdrv_VpeChObj     *chObj)
{
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        for (scalerId = 0U; scalerId < VPS_M2M_SCALER_ID_MAX; scalerId++)
        {
            ...
            {
                if (TRUE == instObj->lazyLoadingEnable[scalerId])
                {
