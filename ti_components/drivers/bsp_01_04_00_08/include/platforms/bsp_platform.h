...
typedef enum
{
    ...
    /**< TDA3XX SOC. */
    BSP_PLATFORM_SOC_ID_DRA7XX = 0x0080U,
    /**< DRA7XX SOC. */
...
/**
 *  \brief Selecte the Clock source for the DISPC Functional Clock.
 *
...
static inline Bool Bsp_platformIsTda2xxFamilyBuild(void)
{
    ...
    if((BSP_PLATFORM_SOC_ID_TDA2XX == socId) ||
       (BSP_PLATFORM_SOC_ID_DRA7XX == socId) ||
...
static inline void BspPlatformInitParams_init(Bsp_PlatformInitParams *initPrms)
{
    if(NULL != initPrms)
    {
        initPrms->isPinMuxSettingReq  = TRUE;
        initPrms->isIrqXBarSettingReq = TRUE;
        initPrms->isDmaXBarSettingReq = TRUE;
        initPrms->isAllMcSPIInitReq   = FALSE;
        initPrms->isAllMcASPInitReq   = TRUE;
    }
