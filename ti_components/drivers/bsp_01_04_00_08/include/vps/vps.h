 ...
 *
 */
static inline void VpsInitParams_init(Vps_InitParams *initPrms);
 ...
 *
 * BSP video Drivers allow users to make use of all video hardware features like
 ...
 *
 * <b>
 * Also refer to BSP driver user guide for detailed features,
 * limitations and usage description.
 * </b>
 *
 * The BSP driver API can be broadly divided into the following categories
 *
 * - <b> FVID2 API </b> (See \ref BSP_DRV_FVID2_API) <br>
 *  API used to create, control and use the different BSP drivers
 *
 ...
 *  - See \ref BSP_DRV_DEVICE_ULTRASONIC_SENSOR_API
 *
 ...
 *
 * - <b> I2C API </b> (See \ref BSP_DRV_I2C_API) <br>
 *  API used to read, write data over I2C interface. I2C interface is used
 *  internally by \ref BSP_DRV_DEVICE_API
 *
 * - <b> UART API </b> (See \ref BSP_DRV_UART_API) <br>
 *  API used to read, write data over UART Terminal
 *
 * - <b> McASP API </b> (See \ref BSP_DRV_MCASP_API) <br>
 *  API used to communicate with audio devices
 *
 * - <b> McSPI API </b> (See \ref BSP_DRV_MCSPI_API) <br>
 *  API used to communicate with flash devices
 *
 * - <b> Audio API </b> (See \ref BSP_DRV_AUDIO_API) <br>
 *  Audio API for devices like mcasp, mcbsp which are connected to the codecs
 *
 * In addition to above drivers the BSP package also includes many sample
 * examples
...
/**
 *  \defgroup BSP_DRV_IOCTL                 BSP - All IOCTL's
 */
