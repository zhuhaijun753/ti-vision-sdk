...
static void App_mainTestTask(void)
{
    ...
    loadRaw( 0x%x, 0, filename, 32, false)\n",
        gAppObj.frames[APP_MESHTBL_FRM_IDX].addr[0U][0U]);
    BspOsal_printf("M2mSimcopLdcVtnfApp: Enter any number to continue\n");
    ...
    for (cnt = 0U; cnt < APP_NUM_FRAMES; cnt++)
    {
        ...
        Bsp_printf(
            "M2mSimcopLdcVtnfApp: Frame [%d] Processing Completed\n", cnt);
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        Bsp_printf("===========================\n");
        Bsp_printf("Iteration  | Time in msecs \n");
