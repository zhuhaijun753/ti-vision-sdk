...
static Int32 LpbkApp_dispConfigDctrl(LpbkApp_Obj *appObj)
{
       ...
       (appObj->testPrms.vencId[0] == VPS_DCTRL_DSS_VENC_LCD1))
    {
        ...
        {
            pixelClk = 54000U;
            retVal   = LpbkApp_configureVideoPllAndClkSrc(pixelClk);
            if(retVal != FVID2_SOK)
            {
                GT_0trace(BspAppTrace, GT_ERR,
                                       ...
                                       UInt32             fieldMerged)
{
        ...
        streamId++)
    {
        for(chId = 0U; chId < instObj->captCreatePrms.numCh; chId++)
        {
            ...
            snprintf(fileStr, 30U, "loopbackOption%dInst%dCh%dStr%d",
                     appObj->testPrms.testId, instObj->captInstId, chId,
                     streamId);
...
Int32 LpbkApp_configureVideoPllAndClkSrc(UInt32 pixelClk)
{
