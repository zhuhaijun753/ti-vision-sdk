...
export Tools_path=$PWD/../ti_components/drivers/starterware_01_04_00_10/bootloader/Tools
export Tools_CRC_path=$PWD/../ti_components/drivers/starterware_01_04_00_10/bootloader/Tools/crc_multicore_image
...
then
    ...
    then
        ...
        "$Tools_path/multicore_image_generator/tda3xx/MulticoreImageGen" LE $Dev_ID $Out_Path/AppImage_UcEarly_LE $IPU1_CPU0 $App_IPU1_CPU0_RPRC $IPU1_CPU1 $App_IPU1_CPU1_RPRC

        # Generate MulticoreImage Gen for UClate (core other than IPU1_0)
        "$Tools_path/multicore_image_generator/tda3xx/MulticoreImageGen" LE $Dev_ID $Out_Path/AppImage_UcLate_LE  $DSP1_CPU $App_DSP1_RPRC $DSP2_CPU $App_DSP2_RPRC $EVE1_CPU $App_EVE1_RPRC $IPU1_CPU_SMP $App_IPU1_RPRC
        else
        # Generate MulticoreImage Gen for UCearly (IPU1_0 only)
        "$Tools_path/multicore_image_generator/tda3xx/MulticoreImageGen" BE $Dev_ID $Out_Path/AppImage_UcEarly_BE $IPU1_CPU0 $App_IPU1_CPU0_RPRC $IPU1_CPU1 $App_IPU1_CPU1_RPRC

        # Generate MulticoreImage Gen for UClate (core other than IPU1_0)
        "$Tools_path/multicore_image_generator/tda3xx/MulticoreImageGen" BE $Dev_ID $Out_Path/AppImage_UcLate_BE  $DSP1_CPU $App_DSP1_RPRC $DSP2_CPU $App_DSP2_RPRC $EVE1_CPU $App_EVE1_RPRC $IPU1_CPU_SMP $App_IPU1_RPRC
        fi
    else
