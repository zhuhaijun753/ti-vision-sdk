...
Int32 SgxDisplayLink_tskRun_PASA(SgxDisplayLink_Obj *pObj, OSA_TskHndl *pTsk,
                            OSA_MsgHndl **pMsg, Bool *done, Bool *ackMsg)
{
    //Added by Sajith - 04/15/16
    //SgxDisplayLink_drvDoProcessFrames_PASA_Out frameOut;
    //---
    Int32 status = SYSTEM_LINK_STATUS_SOK;
    Bool runDone = FALSE, runAckMsg = FALSE;
    OSA_MsgHndl *pRunMsg;
    ...
    *pMsg = NULL;
    ...
    while (!runDone)
    {
        ...
        switch (cmd)
        {
            ...
            case SYSTEM_CMD_NEW_DATA:
                OSA_tskAckOrFreeMsg(pRunMsg, status);
                //frameOut = SgxDisplayLink_drvDoProcessFrames_PASA(pObj);
                //status = frameOut.status;
                status = SgxDisplayLink_drvDoProcessFrames_PASA(pObj);
...
Int32 SgxDisplayLink_tskMain_PASA(struct OSA_TskHndl *pTsk,
                                    OSA_MsgHndl *pMsg, UInt32 curState)
{
    UInt32 cmd = OSA_msgGetCmd(pMsg);
    Bool ackMsg, done;
    ...
    while (!done)
    {
        ...
        switch (cmd)
        {
            ...
            case SYSTEM_CMD_START:
                ...
                {
                    status =
                        SgxDisplayLink_tskRun_PASA(pObj, pTsk, &pMsg, &done, &ackMsg);
                }
 ...
 *******************************************************************************
*/
Int32 SgxDisplayLink_init(void)
{
    Int32 status;
...
Int32 SgxDisplayLink_init_PASA(void)
{
    Int32 status;
    System_LinkObj linkObj;
    UInt32 sgxDisplayId;
 ...
 *******************************************************************************
*/
Int32 SgxDisplayLink_deInit(void)
{
    SgxDisplayLink_Obj *pObj;
    UInt32 sgxDisplayId;
