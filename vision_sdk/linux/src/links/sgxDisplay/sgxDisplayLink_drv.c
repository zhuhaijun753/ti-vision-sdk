...
Int32 SgxDisplayLink_drvDoProcessFrames_PASA(SgxDisplayLink_Obj *pObj)
{
    System_BufferList bufList;
    System_BufferList freeBufList;
    UInt32 freeBufNum, frameIdx;
    ...
    for(frameIdx=0; frameIdx<bufList.numBuf; frameIdx++)
    {
        ...
        if(pBuffer->chNum == 0)
        {
            ...
            if(pBuffer->bufType==SYSTEM_BUFFER_TYPE_VIDEO_FRAME)
            {
                ...
                //running counter for new video frames received
                printf("inBufProcessCount=%d\n",pObj->linkStats.chStats[pBuffer->chNum].inBufProcessCount);

                //save to multiple files
                if (fileFlag == 0)
                {
                    sprintf(fileName,"testImage%03d",pObj->linkStats.chStats[pBuffer->chNum].inBufProcessCount);
                    FILE *f = fopen(fileName,"wb");
                    fwrite(pVideoFrame->bufAddr[0],1,texProp.width*texProp.height,f);
                    fclose(f);
                }
                //overwrite single file
                else if (fileFlag == 1)
                {
                    FILE *f = fopen("testImage","wb");
                    fwrite(pVideoFrame->bufAddr[0],1,texProp.width*texProp.height,f);
                    fclose(f);
                }
                //enable painting frames from camera on display
                if (displayFlag == 1)
                {
                    texYuv[0] = System_eglGetTexYuv(&pObj->eglObj, &texProp, pVideoFrame->bufAddr[0]);
                    if(pObj->createArgs.renderType==SGXDISPLAY_RENDER_TYPE_2x2)
...
Int32 SgxDisplayLink_drvDelete(SgxDisplayLink_Obj *pObj)
{
    ...
    status = System_drmClose(&pObj->eglObj, &pObj->drmObj);
    OSA_assert(status==SYSTEM_LINK_STATUS_SOK);
