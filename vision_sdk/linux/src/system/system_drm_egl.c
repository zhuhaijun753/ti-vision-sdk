...
int System_drmOpen(System_DrmObj *pObj, System_DrmOpenPrms *pPrm)
{
    ...
    drmModeObjectSetProperty(pObj->drmFd, pObj->crtc_id[pObj->curDisplay], DRM_MODE_OBJECT_CRTC , 11, 2);
...
int System_drmClose(System_EglObj *pEglObj, System_DrmObj *pObj)
{
    int i;

    eglMakeCurrent(pEglObj->display,EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
    eglDestroySurface(pEglObj->display, pEglObj->surface);
