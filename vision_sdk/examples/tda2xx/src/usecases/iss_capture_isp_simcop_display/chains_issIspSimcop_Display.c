...
Void Chains_issIspSimcop_Display(Chains_Ctrl *chainsCfg)
{
    ...
    if (CHAINS_ISS_WDR_MODE_SINGLE_PASS == chainsCfg->issWdrMode)
    {
        if(
		(chainsCfg->captureSrc != CHAINS_CAPTURE_SRC_AR0140BAYER_PARALLEL) && 
		(chainsCfg->captureSrc != CHAINS_CAPTURE_SRC_OV10640_CSI2) &&
		(chainsCfg->captureSrc != CHAINS_CAPTURE_SRC_OV10640_PARALLEL) 
        )
        {
              Vps_printf(" CHAINS: Single Pass WDR is only supported on AR0140 & OV10640 !!!\n");
              return;
        }
    }
    ...
    ChainsCommon_SetIssCaptureErrorHandlingCb(chainsCfg->captureSrc,
